    ...
    fn read_neuron_data_multi_channel(&mut self, read_target: &CorticalMappedXYZPNeuronVoxels, _time_of_read: Instant, write_target: &mut Vec<WrappedIOData>, channel_changed: &mut Vec<bool>) -> Result<(), FeagiDataError> {
        ...
        for channel_index in 0..number_of_channels as usize { // Literally not worth making parallel... right?
            ...
            if !(z_a_row_vector_positive.is_empty() && z_a_row_vector_negative.is_empty()) {
                decode_signed_percentage_from_linear_neurons(&z_a_row_vector_positive, &z_a_row_vector_negative, self.channel_dimensions.depth, &mut signed_percentage_4d.a);
            }
            if !(z_b_row_vector_positive.is_empty() && z_b_row_vector_negative.is_empty()) {
                decode_signed_percentage_from_linear_neurons(&z_b_row_vector_positive, &z_b_row_vector_negative, self.channel_dimensions.depth, &mut signed_percentage_4d.b);
            }
            if !(z_c_row_vector_positive.is_empty() && z_c_row_vector_negative.is_empty()) {
                decode_signed_percentage_from_linear_neurons(&z_c_row_vector_positive, &z_c_row_vector_negative, self.channel_dimensions.depth, &mut signed_percentage_4d.c);
            }
            if !(z_d_row_vector_positive.is_empty() && z_d_row_vector_negative.is_empty()) {
                decode_signed_percentage_from_linear_neurons(&z_d_row_vector_positive, &z_d_row_vector_negative, self.channel_dimensions.depth, &mut signed_percentage_4d.d);
            }
        };
