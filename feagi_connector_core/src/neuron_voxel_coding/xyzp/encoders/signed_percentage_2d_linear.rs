...
#[derive(Debug)]
pub struct SignedPercentage2DLinearNeuronVoxelXYZPEncoder {
    channel_dimensions: CorticalChannelDimensions,
    cortical_write_target: CorticalID,
    scratch_space: Vec<((Vec<u32>, Vec<u32>), (Vec<u32>, Vec<u32>))>, // # channels long - (a_pos, a_neg), (b_pos, b_neg)
}

impl NeuronVoxelXYZPEncoder for SignedPercentage2DLinearNeuronVoxelXYZPEncoder {
    ...
    fn write_neuron_data_multi_channel(&mut self, pipelines: &Vec<PipelineStageRunner>, time_of_previous_burst: Instant, write_target: &mut CorticalMappedXYZPNeuronVoxels) -> Result<(), FeagiDataError> {
        ...
        pipelines.par_iter()
            ...
            .try_for_each(|(channel_index, (pipeline, scratch))| -> Result<(), FeagiDataError> {
                ...
                let updated_signed_percentage_2d: SignedPercentage2D = updated_data.try_into()?;

                // scratch arrays get cleared
                encode_signed_percentage_to_linear_neuron_z_index(&updated_signed_percentage_2d.a, z_length_as_float, &mut scratch.0.0, &mut scratch.0.1);
                encode_signed_percentage_to_linear_neuron_z_index(&updated_signed_percentage_2d.b, z_length_as_float, &mut scratch.1.0, &mut scratch.1.1);
...
impl SignedPercentage2DLinearNeuronVoxelXYZPEncoder {
    pub fn new_box(cortical_write_target: CorticalID, z_resolution: NeuronDepth, number_channels: CorticalChannelCount) -> Result<Box<dyn NeuronVoxelXYZPEncoder + Sync + Send>, FeagiDataError> {
        ...
        let encoder = SignedPercentage2DLinearNeuronVoxelXYZPEncoder {
