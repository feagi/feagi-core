    ...
    fn write_neuron_data_multi_channel(&mut self, pipelines: &Vec<PipelineStageRunner>, time_of_previous_burst: Instant, write_target: &mut CorticalMappedXYZPNeuronVoxels) -> Result<(), FeagiDataError> {
        ...
        pipelines.par_iter()
            ...
            .try_for_each(|(channel_index, (pipeline, scratch))| -> Result<(), FeagiDataError> {
                ...
                encode_signed_percentage_to_fractional_exponential_neuron_z_indexes(&updated_signed_percentage_4d.a, self.channel_dimensions.depth, &mut scratch.0.0, &mut scratch.0.1);
                encode_signed_percentage_to_fractional_exponential_neuron_z_indexes(&updated_signed_percentage_4d.b, self.channel_dimensions.depth, &mut scratch.1.0, &mut scratch.1.1);
                encode_signed_percentage_to_fractional_exponential_neuron_z_indexes(&updated_signed_percentage_4d.c, self.channel_dimensions.depth, &mut scratch.2.0, &mut scratch.2.1);
                encode_signed_percentage_to_fractional_exponential_neuron_z_indexes(&updated_signed_percentage_4d.d, self.channel_dimensions.depth, &mut scratch.3.0, &mut scratch.3.1);
