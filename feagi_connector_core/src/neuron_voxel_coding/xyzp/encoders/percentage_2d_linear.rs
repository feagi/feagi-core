...
#[derive(Debug)]
pub struct Percentage2DLinearNeuronVoxelXYZPEncoder {
    channel_dimensions: CorticalChannelDimensions,
    cortical_write_target: CorticalID,
    scratch_space: Vec<(Vec<u32>, Vec<u32>)>, // # channels long
}

impl NeuronVoxelXYZPEncoder for Percentage2DLinearNeuronVoxelXYZPEncoder {
    ...
    fn write_neuron_data_multi_channel(&mut self, pipelines: &Vec<PipelineStageRunner>, time_of_previous_burst: Instant, write_target: &mut CorticalMappedXYZPNeuronVoxels) -> Result<(), FeagiDataError> {
        ...
        pipelines.par_iter()
            ...
            .try_for_each(|(channel_index, (pipeline, scratch))| -> Result<(), FeagiDataError> {
                ...
                encode_unsigned_percentage_to_linear_neuron_z_index(&updated_percentage_2d.a, z_length_as_float, &mut scratch.0);
                encode_unsigned_percentage_to_linear_neuron_z_index(&updated_percentage_2d.b, z_length_as_float, &mut scratch.1);
...
impl Percentage2DLinearNeuronVoxelXYZPEncoder {
    pub fn new_box(cortical_write_target: CorticalID, z_resolution: NeuronDepth, number_channels: CorticalChannelCount) -> Result<Box<dyn NeuronVoxelXYZPEncoder + Sync + Send>, FeagiDataError> {
        ...
        let encoder = Percentage2DLinearNeuronVoxelXYZPEncoder {
