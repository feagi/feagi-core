...
fn main() {
    ...
    println!("  Target frequency:   {} Hz ({:.1} ms/burst)", TARGET_FREQUENCY_HZ, 1000.0 / TARGET_FREQUENCY_HZ);
    println!("  Stimulus neurons:   {}", STIMULUS_NEURONS);
    println!();
    
    // Run profiling
    let metrics = run_profiling(WITH_PROJECTION, NUM_BURSTS, TARGET_FREQUENCY_HZ, STIMULUS_NEURONS);
...
fn run_profiling(with_projection: bool, num_bursts: usize, target_hz: f32, stimulus_count: usize) -> PerformanceMetrics {
    ...
    for burst_num in 0..num_bursts {
        ...
        if (burst_num + 1) % 50 == 0 {
            ...
            println!("  Progress: {:.1}% ({}/{})", progress, burst_num + 1, num_bursts);
        }
    }
...
fn create_genome(with_projection: bool, area_size: (u32, u32, u32)) -> RuntimeGenome {
    ...
    let power_area = CorticalArea::new(
        ...
        2,
        "Power".to_string(),
        Dimensions::new(area_size.0 as usize, area_size.1 as usize, area_size.2 as usize),
        (0, 0, 0),
        AreaType::Memory,
...
fn print_metrics(metrics: &PerformanceMetrics, target_hz: f32) {
    ...
    println!("Total Duration:        {:.2} s", metrics.total_duration.as_secs_f32());
    println!("Total Bursts:          {}", metrics.burst_times.len());
    println!("Achieved Frequency:    {:.2} Hz (target: {:.1} Hz)", achieved_hz, target_hz);
    println!();
    ...
    println!("  Injection (avg):     {:.2} ms ({:.1}%)", avg_inject, (avg_inject / avg_burst) * 100.0);
    println!("  Execution (avg):     {:.2} ms ({:.1}%)", avg_exec, (avg_exec / avg_burst) * 100.0);
}
...
fn analyze_bottlenecks(metrics: &PerformanceMetrics, target_hz: f32) {
    ...
    if avg_burst > target_time {
        ...
        println!("  Over budget by:      {:.2} ms ({:.1}%)", overage, overage_pct);
        println!();
        println!("  Execution phase uses {:.2} ms ({:.1}% of burst)", 
            avg_exec, (avg_exec / avg_burst) * 100.0);
        println!();
        ...
        println!("  System has {:.2} ms headroom ({:.1}%)", headroom, (headroom / target_time) * 100.0);
    }
}
