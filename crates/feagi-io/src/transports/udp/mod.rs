...
use crate::core::{IOError, Result, SharedFBC};
...
impl UdpTransport {
    ...
    async fn send_chunked(&self, data: &[u8]) -> Result<()> {
        let socket = self
            .socket
            .as_ref()
            .ok_or_else(|| IOError::Transport("Socket not initialized".to_string()))?;
        ...
        let peer_addr: SocketAddr = self
            ...
            .peer_address
            .parse()
            .map_err(|e| IOError::Config(format!("Invalid peer address: {}", e)))?;
        ...
        for chunk in chunks {
            socket
                ...
                .await
                .map_err(|e| IOError::Transport(format!("UDP send failed: {}", e)))?;
        }
    ...
    async fn receive_chunked(&self) -> Result<Vec<u8>> {
        ...
        loop {
            let (len, _src_addr) = socket
                .recv_from(&mut buf)
                .await
                .map_err(|e| IOError::Transport(format!("UDP recv failed: {}", e)))?;

            let packet = &buf[..len];

            // Parse header
            let header = PacketHeader::from_bytes(packet)
                .ok_or_else(|| IOError::Transport("Invalid packet header".to_string()))?;
...
impl NonBlockingTransport for UdpTransport {
    ...
    async fn start(&mut self) -> Result<()> {
        ...
        let socket = UdpSocket::bind(&self.config.bind_address)
            .await
            .map_err(|e| IOError::Transport(format!("Failed to bind UDP socket: {}", e)))?;
