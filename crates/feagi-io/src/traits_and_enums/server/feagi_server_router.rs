...
use crate::client_connection_id::ClientConnectionId;
...
pub trait FeagiServerRouter: FeagiServer {
    fn try_poll_receive(&mut self) -> Result<Option<(ClientConnectionId, &[u8])>, FeagiNetworkError>;

    fn send_response(&mut self, client: ClientConnectionId, response: &[u8])
