...
fn test_zmq_host() -> String {
    std::env::var("FEAGI_TEST_ZMQ_HOST")
        .expect("FEAGI_TEST_ZMQ_HOST is required (set to a connectable host, e.g. 127.0.0.1)")
}
...
fn core_zmq_pub_sub_roundtrip() {
    ...
    let mut publisher = ZmqPub::new(runtime.clone(), ServerConfig::new(endpoint.clone()))
        .expect("Failed to create ZmqPub");
    publisher.start().expect("Failed to start ZmqPub");

    let mut subscriber = ZmqSub::new(runtime.clone(), ClientConfig::new(endpoint.clone()))
        .expect("Failed to create ZmqSub");
    subscriber.start().expect("Failed to start ZmqSub");
    subscriber.subscribe(b"topic").expect("Failed to subscribe");
...
fn core_zmq_push_pull_roundtrip() {
    ...
    let mut puller = ZmqPull::new(runtime.clone(), ServerConfig::new(endpoint.clone()))
        .expect("Failed to create ZmqPull");
    puller.start().expect("Failed to start ZmqPull");

    let mut pusher = ZmqPush::new(runtime.clone(), ClientConfig::new(endpoint.clone()))
    ...
    let mut attempts = 0;
    let recv_payload = wait_for(timeout, || match puller.pull_timeout(poll_timeout_ms) {
        Ok(value) => Some(value),
        Err(TransportError::Timeout) => {
            if attempts < 3 {
...
fn core_zmq_router_dealer_roundtrip() {
    ...
    let mut router = ZmqRouter::new(runtime.clone(), ServerConfig::new(endpoint.clone()))
        .expect("Failed to create ZmqRouter");
    router.start().expect("Failed to start ZmqRouter");

    let mut dealer = ZmqDealer::new(runtime.clone(), ClientConfig::new(endpoint.clone()))
    ...
    reply_handle.send(response).expect("Router response failed");

    let dealer_response = dealer_thread.join().expect("Dealer thread panicked");
...
fn zmq_rest_registration_roundtrip() {
    ...
    let mut handler = RegistrationHandler::new(
        registry,
        registration_port,
        sensory_port,
        motor_port,
    ...
    rest_stream
        .start()
        .expect("Failed to start ZMQ REST stream");
    ...
    let capabilities_struct = feagi_io::core::AgentCapabilities {
        sensory: Some(feagi_io::core::SensoryCapability {
            rate_hz: 1.0,
            shm_path: None,
        }),
        ..Default::default()
    ...
    let capabilities =
        serde_json::to_value(capabilities_struct).expect("Failed to serialize AgentCapabilities");
...
fn zmq_rest_heartbeat_and_deregister_roundtrip() {
    ...
    let register_response = send_rest_request(
        runtime.as_ref(),
        &mut dealer,
        timeout_window,
        register_request,
    ...
    let heartbeat_response = send_rest_request(
        runtime.as_ref(),
        &mut dealer,
        timeout_window,
        heartbeat_request,
    ...
    let deregister_response = send_rest_request(
        runtime.as_ref(),
        &mut dealer,
        timeout_window,
        deregister_request,
...
fn io_api_zmq_pub_sub_roundtrip() {
    ...
    let mut publisher = FEAGIZMQServerPublisher::new(endpoint.clone(), Box::new(|_| {}))
        .expect("Failed to create FEAGIZMQServerPublisher");
    publisher.start().expect("Failed to start publisher");

    let mut subscriber = FEAGIZMQClientSubscriber::new(endpoint.clone(), Box::new(|_| {}))
    ...
    publisher.publish(payload).expect("Failed to publish data");

    let mut attempts = 0;
    let recv_payload = wait_for(timeout, || match subscriber.try_poll_receive() {
        Ok(Some(value)) => Some(value.to_vec()),
        Ok(None) => {
            if attempts < 3 {
                publisher.publish(payload).expect("Retry publish failed");
            }
            attempts += 1;
            None
        }
...
fn io_api_zmq_push_pull_roundtrip() {
    ...
    let mut puller = FEAGIZMQServerPuller::new(endpoint.clone(), Box::new(|_| {}))
        .expect("Failed to create FEAGIZMQServerPuller");
    puller.start().expect("Failed to start puller");

    let mut pusher = FEAGIZMQClientPusher::new(endpoint.clone(), Box::new(|_| {}))
        .expect("Failed to create FEAGIZMQClientPusher");
    pusher.connect(&endpoint).expect("Failed to connect pusher");

    let payload = b"io-api-push-pull";
    pusher.push_data(payload);

    let mut attempts = 0;
    let recv_payload = wait_for(timeout, || match puller.try_poll_receive() {
        Ok(Some(value)) => Some(value.to_vec()),
        Ok(None) => {
            if attempts < 3 {
                pusher.push_data(payload);
...
fn io_api_zmq_router_requester_roundtrip() {
    ...
    let mut router = FEAGIZMQServerRouter::new(endpoint.clone(), Box::new(|_| {}))
        .expect("Failed to create FEAGIZMQServerRouter");
    router.start().expect("Failed to start router");

    let mut requester = FEAGIZMQClientRequester::new(endpoint.clone(), Box::new(|_| {}))
