...
//!
//! Relocated from the `feagi-npu-plasticity` crate, which the NPU rewrite dissolved. It lives in
//! the service layer because it is read-side observability state: the health-check endpoint
//! reports it and `ApiState` holds it, while the writer is whichever service owns plasticity.
//!
