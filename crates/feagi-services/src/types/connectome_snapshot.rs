...
//! Connectome snapshot DTOs.
//!
//! These types describe the serializable state of a complete connectome. They previously lived in
//! `feagi-npu-neural::types::connectome`, which was dissolved by the NPU rewrite. They are service
//! layer DTOs rather than NPU types: the REST surface serializes them directly
//! (`POST /v1/connectome/upload`) and `crate::connectome` writes them to disk with bincode, so the
//! field names, ordering, and `serde` attributes are part of the external contract and must not be
//! changed without a format version bump.

use ahash::AHashMap;
use serde::{Deserialize, Serialize};

/// Neuron state, laid out as parallel arrays indexed by neuron id.
...
impl SerializableNeuronArray {
    /// Create a new empty neuron array with every parallel array pre-sized to `capacity`.
...
/// Synapse state, laid out as parallel arrays indexed by synapse id.
...
impl SerializableSynapseArray {
    /// Create a new empty synapse array with every parallel array pre-sized to `capacity`.
    pub fn new(capacity: usize) -> Self {
        Self {
            count: 0,
            capacity,
            ...
            weights: vec![0.0f32; capacity],
            postsynaptic_potentials: vec![0.0f32; capacity],
            types: vec![0; capacity],
            delay_bursts: vec![1; capacity],
            valid_mask: vec![false; capacity],
            source_index: AHashMap::new(),
        }
    }
...
///
/// This structure captures the entire state of an NPU, including:
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        for i in 0..self.synapses.count {
            ...
            if source >= self.neurons.count {
                return Err(format!("Synapse {} has invalid source neuron: {}", i, source));
            }

            if target >= self.neurons.count {
                return Err(format!("Synapse {} has invalid target neuron: {}", i, target));
            }
        }
