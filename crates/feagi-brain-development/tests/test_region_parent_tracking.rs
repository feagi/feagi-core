...
fn create_root_region() -> BrainRegion {
    ...
    BrainRegion::new(root_region_id, "Root Brain Region".to_string(), RegionType::Undefined).expect("Failed to create root region")
}
...
fn create_subregion() -> BrainRegion {
    ...
    BrainRegion::new(subregion_id, "Subregion".to_string(), RegionType::Undefined).expect("Failed to create subregion")
}
...
fn test_regression_parent_region_exists() {
    ...
    let parent_id_opt = hierarchy.get_parent(&subregion_id_str);
    assert!(parent_id_opt.is_some(), "CRITICAL: Parent lookup should succeed - this was the bug!");
...
fn test_regions_stored_by_uuid_not_hardcoded() {
    ...
    assert!(by_hardcoded.is_none(), "Should NOT be retrievable by hardcoded 'root' string");
