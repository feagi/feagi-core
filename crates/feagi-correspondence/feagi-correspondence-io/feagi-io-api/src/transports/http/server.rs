...
#[cfg(feature = "feagi-agent")]
use feagi_io::protocol_implementations::websocket::websocket_std::{FeagiWebSocketServerPublisherProperties, FeagiWebSocketServerPullerProperties};
#[cfg(feature = "feagi-agent")]
use feagi_io::protocol_implementations::zmq::{FeagiZmqServerPublisherProperties, FeagiZmqServerPullerProperties};
...
pub struct ApiState {
    ...
    pub network_connection_info_provider: Option<Arc<dyn crate::endpoints::network::NetworkConnectionInfoProvider>>,
...
impl ApiState {
    ...
    #[cfg(feature = "feagi-agent")]
    pub fn init_agent_registration_handler() -> Arc<std::sync::Mutex<feagi_agent::server::FeagiAgentHandler>> {
        ...
        let mut handler =
            feagi_agent::server::FeagiAgentHandler::new_with_liveness_config(Box::new(feagi_agent::server::auth::DummyAuth {}), liveness_config);
        let available_transports: Vec<String> = config.transports.available.iter().map(|transport| transport.to_lowercase()).collect();

        if available_transports.iter().any(|transport| transport == "zmq") {
            let sensory_address = format_tcp_endpoint(&config.zmq.bind_host, config.ports.zmq_sensory_port);
            let sensory_adv_address = format_tcp_endpoint(&config.zmq.advertised_host, config.ports.zmq_sensory_port);
            let motor_address = format_tcp_endpoint(&config.zmq.bind_host, config.ports.zmq_motor_port);
            let motor_adv_address = format_tcp_endpoint(&config.zmq.advertised_host, config.ports.zmq_motor_port);
            let visualization_address = format_tcp_endpoint(&config.zmq.bind_host, config.ports.zmq_visualization_port);
            let visualization_adv_address = format_tcp_endpoint(&config.zmq.advertised_host, config.ports.zmq_visualization_port);

            let sensory =
                FeagiZmqServerPullerProperties::new(&sensory_address, &sensory_adv_address).expect("Failed to create ZMQ sensory puller properties");
            handler.add_puller_server(Box::new(sensory));

            let motor =
                FeagiZmqServerPublisherProperties::new(&motor_address, &motor_adv_address).expect("Failed to create ZMQ motor publisher properties");
            let visualization = FeagiZmqServerPublisherProperties::new(&visualization_address, &visualization_adv_address)
        ...
        if available_transports.iter().any(|transport| transport == "websocket" || transport == "ws") {
            let sensory_address = format_ws_address(&config.websocket.bind_host, config.websocket.sensory_port);
            let sensory_adv_address = format_ws_address(&config.websocket.advertised_host, config.websocket.sensory_port);
            let motor_address = format_ws_address(&config.websocket.bind_host, config.websocket.motor_port);
            let motor_adv_address = format_ws_address(&config.websocket.advertised_host, config.websocket.motor_port);
            let visualization_address = format_ws_address(&config.websocket.bind_host, config.websocket.visualization_port);
            let visualization_adv_address = format_ws_address(&config.websocket.advertised_host, config.websocket.visualization_port);

            let sensory = FeagiWebSocketServerPullerProperties::new_with_remote(&sensory_address, &sensory_adv_address)
                .expect("Failed to create WebSocket sensory puller properties");
            handler.add_puller_server(Box::new(sensory));

            let motor = FeagiWebSocketServerPublisherProperties::new(&motor_address, &motor_adv_address)
                .expect("Failed to create WebSocket motor publisher properties");
            let visualization = FeagiWebSocketServerPublisherProperties::new(&visualization_address, &visualization_adv_address)
    ...
    pub fn init_genome_transition_controls() -> (Arc<tokio::sync::Mutex<()>>, Arc<AtomicBool>) {
        (Arc::new(tokio::sync::Mutex::new(())), Arc::new(AtomicBool::new(false)))
    }
...
pub fn create_http_server(state: ApiState) -> Router {
    ...
    Router::new()
        ...
        .with_state(state)
        // Add middleware
        .layer(middleware::from_fn_with_state(middleware_state, reject_during_genome_transition))
        ...
        .layer(
            TraceLayer::new_for_http()
                ...
                .on_eos(
                    |_trailers: Option<&axum::http::HeaderMap>, stream_duration: std::time::Duration, _span: &tracing::Span| {
...
async fn reject_during_genome_transition(State(state): State<ApiState>, request: Request<Body>, next: Next) -> Response {
    ...
    if transition_in_progress && !is_transition_allowed_route(path, method) {
        let payload = crate::common::ApiError::conflict("Genome transition in progress: request rejected to preserve deterministic load semantics");
...
fn is_transition_allowed_route(path: &str, method: &Method) -> bool {
    if path.starts_with("/v1/genome/upload") || path.starts_with("/v1/genome/load") || path.starts_with("/v1/genome/amalgamation") {
        return true;
    }
...
fn create_v1_router() -> Router<ApiState> {
    ...
    Router::new()
        ...
        .route("/agent/manual_stimulation", axum::routing::post(agent::manual_stimulation))
        .route("/agent/fq_sampler_status", get(agent::get_fq_sampler_status))
        .route("/agent/capabilities", get(agent::get_capabilities))
        .route("/agent/capabilities/all", get(agent::get_all_agent_capabilities))
        ...
        .route(
            ...
            get(system::get_cortical_area_visualization_skip_rate).put(system::set_cortical_area_visualization_skip_rate),
        )
        .route(
            "/system/cortical_area_visualization_suppression_threshold",
            get(system::get_cortical_area_visualization_suppression_threshold).put(system::set_cortical_area_visualization_suppression_threshold),
        )
        ...
        .route("/cortical_area/memory", get(cortical_area::get_memory_cortical_area))
        .route(
        ...
        .route(
            ...
            get(cortical_area::get_mapping_restrictions_query).post(cortical_area::post_mapping_restrictions),
        )
        ...
        .route("/morphology/rename", axum::routing::put(morphology::put_rename_morphology))
        .route(
        ...
        .route("/connectome/neuron_properties_at", get(connectome::get_neuron_properties_at_query))
        .route("/connectome/memory_neuron", get(connectome::get_memory_neuron))
        ...
        .route("/burst_engine/fire_queue/neuron", get(burst_engine::get_fire_queue_neuron))
        ...
        .route(
            ...
            get(burst_engine::get_fire_ledger_default_window_size).put(burst_engine::put_fire_ledger_default_window_size),
        )
        .route(
        ...
        .route("/monitoring/cortical_activity", get(monitoring::get_cortical_activity))
        ...
        .route("/network/connection_info", get(network::get_connection_info))
}
...
async fn log_request_response_bodies(request: Request<Body>, next: Next) -> Result<Response, StatusCode> {
