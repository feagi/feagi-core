...
use feagi_agent::clients::{SessionAction, SessionEvent, SessionInit, SessionPhase, SessionStateMachine, SessionTimingConfig};
...
fn make_init() -> SessionInit {
    let agent_descriptor = AgentDescriptor::new("neuraville", "test_agent", 1).expect("valid descriptor");
    let auth_token = AuthToken::new([0u8; 32]);
    SessionInit {
        agent_descriptor,
        auth_token,
        requested_capabilities: vec![AgentCapabilities::SendSensorData, AgentCapabilities::ReceiveMotorData],
...
fn registration_success_without_required_endpoints_fails() {
    ...
    use feagi_agent::command_and_control::agent_registration_message::{AgentRegistrationMessage, RegistrationResponse};
    ...
    let msg = FeagiMessage::AgentRegistration(AgentRegistrationMessage::ServerRespondsRegistration(RegistrationResponse::Success(
        feagi_io::AgentID::new_blank(),
        empty,
    )));
