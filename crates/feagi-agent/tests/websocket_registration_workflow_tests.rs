#![cfg(all(feature = "agent-server", feature = "agent-client", feature = "agent-transport-websocket-std"))]
...
use feagi_io::protocol_implementations::websocket::websocket_std::{
    FeagiWebSocketClientRequesterProperties, FeagiWebSocketServerPublisherProperties, FeagiWebSocketServerPullerProperties,
...
fn websocket_registration_workflow_succeeds_end_to_end() {
    ...
    let mut handler = FeagiAgentHandler::new_with_liveness_config(Box::new(DummyAuth {}), AgentLivenessConfig::default());
    ...
    let client_thread = thread::spawn(move || {
        let result: Result<(AgentID, String), String> = (|| {
            ...
            client.request_connect().map_err(|e| format!("client failed to connect: {e}"))?;
            client
                .request_registration(
                    AgentDescriptor::new("neuraville", "ws-registration-test", 1).map_err(|e| format!("invalid descriptor: {e}"))?,
            ...
            loop {
                client.poll_for_messages().map_err(|e| format!("client poll_for_messages failed: {e}"))?;

                if let AgentRegistrationStatus::Registered(agent_id, endpoints) = client.registration_status() {
                    let viz_endpoint = endpoint_as_string(
                        endpoints
                            .get(&AgentCapabilities::ReceiveNeuronVisualizations)
                            .ok_or_else(|| "missing visualization endpoint in registration response".to_string())?,
                    );
    ...
    let deadline = Instant::now() + Duration::from_secs(5);
    loop {
        handler.poll_command_and_control().expect("server poll_command_and_control failed");

        if let Ok(client_result) = client_result_rx.try_recv() {
            let (agent_id, returned_viz_endpoint) = client_result.expect("client registration flow failed");
            ...
            let (descriptor, capabilities) = registered_agents.get(&agent_id).expect("registered agent should exist");
        ...
        if Instant::now() >= deadline {
            ...
            panic!("timed out waiting for websocket registration response while server was polling");
        }
...
fn websocket_registration_times_out_when_server_does_not_respond() {
    ...
    let router_props = FeagiWebSocketServerRouterProperties::new_with_remote(&registration_bind, &registration_remote)
        .expect("failed to create websocket router properties");
    let mut router = router_props.as_boxed_server_router();
    router.request_start().expect("failed to start websocket router");
    ...
    let client_thread = thread::spawn(move || {
        let result: Result<(), String> = (|| {
            ...
            client
                .request_registration(
                    AgentDescriptor::new("neuraville", "ws-no-reply-test", 1).map_err(|e| format!("invalid descriptor: {e}"))?,
                    AuthToken::new([0u8; 32]),
                    vec![AgentCapabilities::ReceiveNeuronVisualizations],
                )
                .map_err(|e| format!("client failed to send registration request: {e}"))?;
    ...
    loop {
        match router.poll().clone() {
            FeagiEndpointState::ActiveHasData => {
                ...
                let (_session_id, _payload) = router.consume_retrieved_request().expect("failed to consume websocket request");
                saw_request = true;
            }
        ...
        if let Ok(client_result) = client_result_rx.try_recv() {
            ...
            assert!(saw_request, "server should receive the registration request in timeout scenario");
            client_thread.join().expect("client thread panicked");
            break;
        }
...
fn websocket_registration_returns_all_requested_capability_endpoints() {
    ...
    let registration_remote_for_client = registration_remote.clone();
    let (client_result_tx, client_result_rx) = mpsc::channel::<Result<(AgentID, String, String, String), String>>();
    let client_thread = thread::spawn(move || {
        let result: Result<(AgentID, String, String, String), String> = (|| {
            ...
            client
                .request_registration(
                    AgentDescriptor::new("neuraville", "ws-registration-all-caps-test", 1).map_err(|e| format!("invalid descriptor: {e}"))?,
                    AuthToken::new([0u8; 32]),
                    vec![
    ...
    loop {
        handler.poll_command_and_control().expect("server poll_command_and_control failed");
        if let Ok(client_result) = client_result_rx.try_recv() {
            let (agent_id, sensory_endpoint, motor_endpoint, viz_endpoint) = client_result.expect("client all-capabilities registration failed");
            ...
            assert_eq!(viz_endpoint, visualization_remote, "visualization endpoint mismatch");

            let registered_agents = handler.get_all_registered_agents();
            let (_descriptor, capabilities) = registered_agents.get(&agent_id).expect("registered agent should exist");
...
fn websocket_deregistration_removes_registered_agent_from_server() {
    ...
    let client_thread = thread::spawn(move || {
        let result: Result<AgentID, String> = (|| {
            ...
            client
                .request_registration(
                    AgentDescriptor::new("neuraville", "ws-deregistration-test", 1).map_err(|e| format!("invalid descriptor: {e}"))?,
                    AuthToken::new([0u8; 32]),
                    vec![AgentCapabilities::ReceiveNeuronVisualizations],
                )
                .map_err(|e| format!("client failed to send registration request: {e}"))?;
