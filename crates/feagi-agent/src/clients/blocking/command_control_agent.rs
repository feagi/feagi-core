use crate::command_and_control::agent_registration_message::{
    AgentRegistrationMessage, DeregistrationRequest, DeregistrationResponse, RegistrationRequest, RegistrationResponse,
};
...
impl CommandControlAgent {
    ...
    pub fn request_connect(&mut self) -> Result<(), FeagiAgentError> {
        if self.registration_status != AgentRegistrationStatus::NotRegistered {
            return Err(FeagiAgentError::connection_failed("Agent already connected and registered!".to_string()));
        }
        ...
        match requester.poll() {
            ...
            _ => {
                self.requester = Some(requester);
                Err(FeagiAgentError::connection_failed("Socket is already active!".to_string()))
            }
        }
    }
    ...
    pub fn request_registration(
        ...
        let request = RegistrationRequest::new(agent_descriptor, auth_token, requested_capabilities, transport_protocol);

        let request_message = FeagiMessage::AgentRegistration(AgentRegistrationMessage::ClientRequestRegistration(request));
    ...
    pub fn request_deregistration(
        ...
        let message = FeagiMessage::AgentRegistration(AgentRegistrationMessage::ClientRequestDeregistration(request));
    ...
    pub fn request_disconnect(&mut self) -> Result<(), FeagiAgentError> {
        let requester = self
            .requester
            .as_mut()
            .ok_or_else(|| FeagiAgentError::connection_failed("No socket is active to disconnect!".to_string()))?;
    ...
    pub fn poll_for_messages(&mut self) -> Result<(&FeagiEndpointState, Option<FeagiMessage>), FeagiAgentError> {
        let maybe_message = {
            ...
            match state_snapshot {
                FeagiEndpointState::Inactive | FeagiEndpointState::Pending | FeagiEndpointState::ActiveWaiting => Ok(None),
                FeagiEndpointState::ActiveHasData => {
                    let data = requester.consume_retrieved_response()?;
                    self.request_buffer.try_write_data_by_copy_and_verify(data)?;
                    let feagi_message: FeagiMessage = (&self.request_buffer).try_into()?;

                    match &feagi_message {
                        FeagiMessage::HeartBeat => Ok(Some(FeagiMessage::HeartBeat)),
                        FeagiMessage::AgentRegistration(registration_message) => match registration_message {
                            AgentRegistrationMessage::ClientRequestRegistration(_) => {
                                Err(FeagiAgentError::connection_failed("Client cannot register agents!".to_string()))
                            }
                            AgentRegistrationMessage::ServerRespondsRegistration(registration_response) => match registration_response {
                                RegistrationResponse::FailedInvalidRequest => {
                                    Err(FeagiAgentError::connection_failed("Invalid server responses!".to_string()))
                                }
                                RegistrationResponse::FailedInvalidAuth => Err(FeagiAgentError::connection_failed("Invalid auth token!".to_string())),
                                RegistrationResponse::AlreadyRegistered => {
                                    Err(FeagiAgentError::connection_failed("Client already registered!".to_string()))
                                }
                                RegistrationResponse::Success(session_id, endpoints) => {
                                    self.registration_status = AgentRegistrationStatus::Registered(*session_id, endpoints.clone());
                                    Ok(Some(feagi_message))
                                }
                            },
                            AgentRegistrationMessage::ClientRequestDeregistration(_) => Err(FeagiAgentError::connection_failed(
                                "Client cannot receive deregistration request from server!".to_string(),
                            )),
                            AgentRegistrationMessage::ServerRespondsDeregistration(deregistration_response) => match deregistration_response {
                                DeregistrationResponse::Success => {
                                    requester.request_disconnect()?;
                                    self.registration_status = AgentRegistrationStatus::NotRegistered;
                                    Ok(Some(feagi_message))
                                }
                                DeregistrationResponse::NotRegistered => Ok(Some(feagi_message)),
                            },
                        },
                ...
                FeagiEndpointState::Errored(_) => {
                    ...
                    Err(FeagiAgentError::connection_failed("Error occurred".to_string()))
                }
            }
        }?;
    ...
    pub fn send_message(&mut self, message: FeagiMessage, increment_value: u16) -> Result<(), FeagiAgentError> {
        let agent_id = match &self.registration_status {
            ...
            AgentRegistrationStatus::NotRegistered => {
                ...
                match &message {
                    FeagiMessage::AgentRegistration(AgentRegistrationMessage::ClientRequestRegistration(_)) => AgentID::new_blank(),
                    _ => {
...
mod tests {
    ...
    impl feagi_io::traits_and_enums::client::FeagiClientRequester for DummyRequester {
        ...
        fn consume_retrieved_response(&mut self) -> Result<&[u8], feagi_io::FeagiNetworkError> {
            Err(feagi_io::FeagiNetworkError::ReceiveFailed("dummy requester has no responses".to_string()))
        }

        fn as_boxed_requester_properties(&self) -> Box<dyn feagi_io::traits_and_enums::client::FeagiClientRequesterProperties> {
    ...
    fn registration_request_can_be_sent_before_registration() {
        let endpoint = TransportProtocolEndpoint::Zmq(ZmqUrl::new("tcp://example:1").expect("valid dummy endpoint"));
