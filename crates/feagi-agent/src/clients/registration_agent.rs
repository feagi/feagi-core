...
impl RegistrationAgent {
    ...
    pub async fn try_register(&mut self, registration_request: RegistrationRequest) -> Result<(SessionID, HashMap<AgentCapabilities, String>), FeagiAgentClientError> {
        ...
        match response {
            ...
            RegistrationResponse::FailedInvalidAuth => Err(FeagiAgentClientError::ConnectionFailed(
                "Server rejected authentication!".to_string()
            )),
            RegistrationResponse::AlreadyRegistered => Err(FeagiAgentClientError::ConnectionFailed(
    ...
    pub async fn disconnect(&mut self) -> Result<(), FeagiAgentClientError> {
