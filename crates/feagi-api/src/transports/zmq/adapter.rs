...
impl ZmqApiAdapter {
                ...
                match result {
                    Ok((request_data, reply_handle)) => {
                        ...
                        let api_request: ApiRequest = match serde_json::from_slice(&request_data) {
                            Ok(req) => req,
                            Err(e) => {
                                let error_response = ApiResponse::<()>::error(ApiError::bad_request(
    ...
    fn route_request(
        ...
        match (request.method.as_str(), request.path.as_str()) {
            ...
            // Not found
            _ => {
                ApiResponse::error(ApiError::not_found(
                    &format!("Endpoint not found: {} {}", request.method, request.path),
                ))
            }
        }
    }
        ...
        match result {
            Ok(response) => {
                ...
                match serde_json::to_value(&response.data) {
                    ...
                    Err(e) => ApiResponse::error(ApiError::internal_error(
