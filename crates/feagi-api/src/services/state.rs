...
pub fn create_api_state_from_genome(genome: SharedGenome, npu: OptionalNpu, version_info: feagi_services::types::VersionInfo) -> ApiState {
    let connectome_service = Arc::new(GenomeConnectomeService::new(Arc::clone(&genome), npu.clone()));
    let genome_service = Arc::new(GenomeGenomeService::new(Arc::clone(&genome), npu.clone()));
    let analytics_service = Arc::new(GenomeAnalyticsService::new(Arc::clone(&genome), npu.clone()));
    ...
    ApiState {
        ...
        feagi_session_timestamp: std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_millis() as i64,
