...
use crate::quantizable::spatial::SpatialUnsignedBaseXDTrait;
...
pub fn verify_linear_index_within_unsigned_3d_bounds<QuantIndex: QuantizedIndexCountTrait>(x: QuantIndex, y: QuantIndex, z: QuantIndex) -> Result<(), FeagiDataQuantizedError> {
    let xy = x.to_usize() * y.to_usize();
    FeagiDataQuantizedError::verify_quantization_index::<QuantIndex>(xy, "3D spatial coordinates would exceed linear quantization index!")?;
    FeagiDataQuantizedError::verify_quantization_index::<QuantIndex>(xy * z.to_usize(), "3D spatial coordinates would exceed linear quantization index!")
}
