...
use common::{create_minimal_test_connectome, create_temp_dir};
use feagi_connectome_serialization::{load_connectome, save_connectome};
...
fn test_connectome_save_load_roundtrip() {
    ...
    save_connectome(&connectome, &connectome_path).expect("Failed to save connectome");

    // Verify file exists
    assert!(connectome_path.exists(), "Connectome file should exist");

    // Load it back
    let loaded = load_connectome(&connectome_path).expect("Failed to load connectome");
...
fn test_connectome_metadata_preservation() {
    ...
    assert_eq!(loaded.metadata.tags.get("test"), Some(&"true".to_string()));
}
...
fn test_neuron_coordinate_lookup() {
    ...
    let coords = vec![(0u32, 0u32, 0u32), (1u32, 1u32, 1u32), (2u32, 2u32, 2u32)];

    let cortical_area_id = 0u32;
    let neuron_ids = npu
        .neuron_array
        .batch_coordinate_lookup(cortical_area_id, &coords);
