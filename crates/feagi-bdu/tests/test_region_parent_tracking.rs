...
fn test_root_region_uuid_storage() {
    ...
    hierarchy.add_region(root_region, None).expect("Failed to add root region");
    
    // Retrieve by UUID string
    let retrieved = hierarchy.get_region(&root_id_str);
    assert!(retrieved.is_some(), "Root region should be retrievable by UUID string");
...
fn test_subregion_parent_uuid_reference() {
    ...
    hierarchy.add_region(subregion, Some(root_id_str.clone())).expect("Failed to add subregion");
    ...
    assert_eq!(parent_id_opt.unwrap(), &root_id_str, "Parent ID should match root region UUID");
}
...
fn test_multiple_subregions_same_parent() {
    ...
    for i in 0..3 {
        ...
        hierarchy.add_region(subregion, Some(root_id_str.clone())).expect(&format!("Failed to add subregion {}", i));
        
        // Verify each can find its parent
        let parent_id_opt = hierarchy.get_parent(&subregion_id_str);
        assert!(parent_id_opt.is_some(), "Subregion {} should have parent", i);
        let parent_id_str = parent_id_opt.unwrap();
        assert_eq!(parent_id_str, &root_id_str, "Subregion {} parent should be root", i);
        
        // Get the actual parent region
        let parent_region = hierarchy.get_region(parent_id_str);
        assert!(parent_region.is_some(), "Subregion {} parent region should exist", i);
    }
}
...
fn test_deep_hierarchy_grandchild() {
    ...
    let root_id = root_region.region_id.clone();
    let root_id_str = root_id.to_string();
    hierarchy.add_region(root_region, None).expect("Failed to add root");
    
    // Create child
    let child_region = create_subregion();
    let child_id = child_region.region_id.clone();
    let child_id_str = child_id.to_string();
    hierarchy.add_region(child_region, Some(root_id_str.clone())).expect("Failed to add child");
    
    // Create grandchild
    let grandchild_region = create_subregion();
    let grandchild_id_str = grandchild_region.region_id.to_string();
    hierarchy.add_region(grandchild_region, Some(child_id_str.clone())).expect("Failed to add grandchild");
    ...
    let parent_id_str = parent_id_opt.unwrap();
    assert_eq!(parent_id_str, &child_id_str, "Grandchild parent should be child");
    ...
    assert_eq!(root_parent_id_str, &root_id_str, "Child parent should be root");
}
...
fn test_regions_stored_by_uuid_not_hardcoded() {
    ...
    assert!(by_uuid.is_some(), 
           "Should be retrievable by UUID string");
