    ...
    FGQ: FeagiGlobalQuantization,
{
    fn process_phase(data: &mut BurstEngineDataRayon<FGQ>) -> BurstEngineJustCompletedPhase {
        ...
        let neuron_engine_cortical_indexes =
            ParallelSlicePtr::new(&data.neuron_engine_cortical_indexes);
        let cortical_context_lookups = ParallelSlicePtr::new(&data.cortical_context_lookups);
        let cortical_layouts_dimensional =
            ParallelSlicePtr::new(&data.cortical_layouts.dimensional);
    ...
    FGQ: FeagiGlobalQuantization,
{
    fn process_phase(data: &mut BurstEngineDataRayon<FGQ>) -> BurstEngineJustCompletedPhase {
        ...
        data.neuron_fcls
            ...
            .par_iter_mut()
            .enumerate()
            .for_each(|(neuron_type_index_u, fcl_value)| {
                if *fcl_value == NPUWrappedNeuronMembranePotential::QUANT_ZERO {
                    return;
                }
                ...
                unsafe {
                    ...
                    let cortical_context_lookup = cortical_context_lookups
                        .get(engine_cortical_area_index_of_neuron.to_usize());

                    let neuron_index_local =
                        NPUWrappedNeuronCorticalLocalIndex::wrap(neuron_mp_quant_index.unwrap())
                            - cortical_context_lookup.mp_quant_to_local_neuron_index_offset;

                    let neuron_history = neuron_history.get_mut(
                        cortical_context_lookup
                            .mp_quant_to_neuron_history_index_offset
                            .to_usize(),
                    );

                    // TODO not always dimensional!
                    let dimensional = cortical_layouts_dimensional
                        .get(cortical_context_lookup.cortical_layout_index.to_usize());
                    let feagi_standard_cortical_data = neuron_model_cortical_data.get(
                        cortical_context_lookup
                            .neuron_model_cortical_data_index
                            .to_usize(),
                    );

                    let neuron_model_data =
                        neuron_model_neuron_data.get_mut(neuron_mp_quant_index_usize);
                    let neuron_potential = neuron_potentials.get_mut(neuron_mp_quant_index_usize);
                }
            });
