...
//!
//! Relocated from the `feagi-npu-plasticity` crate, which the NPU rewrite dissolved. The ID
//! ranges are part of the REST contract (`/v1/connectome/memory_neuron` validates against them
//! and reports the bounds in its error message), so the constants must keep their values.
