...
fn test_projector_psp_division_issue() {
    ...
    for (i, &target) in neurons_b.iter().enumerate() {
        ...
        println!("Synapse {}: {} -> {} (weight=1, conductance=1)", i, neuron_a.0, target.0);
    }
    ...
    let area_a_id = CorticalID::try_from_base_64(&CoreCorticalType::Death.to_cortical_id().as_base_64()).unwrap();
    ...
    for (i, &neuron) in neurons_b.iter().enumerate() {
        let mp = npu.get_neuron_property_by_index(neuron.0 as usize, "membrane_potential")
            .expect("Should have MP");
        let fired = result2.fired_neurons.contains(&neuron);
        println!("  Neuron B[{}] (ID {}): MP={:.2}, Fired={}", i, neuron.0, mp, fired);
    }
    ...
    let fired_count = neurons_b.iter().filter(|n| result2.fired_neurons.contains(n)).count();
    
    println!("\n=== ANALYSIS ===");
    println!("PSP division: 1 / 10 synapses = {} (integer division)", 1u8 / 10u8);
    ...
    assert_eq!(fired_count, 0, "NO neurons in B should fire when PSP is divided to 0");
}
