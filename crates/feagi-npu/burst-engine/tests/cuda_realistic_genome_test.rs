...
mod cuda_realistic_genome_tests {
    ...
    fn create_realistic_genome() -> (NeuronArray<f32>, SynapseArray) {
        ...
        println!(
            "   Area A: {} neurons (512x512, 5% excitability)",
            area_a_neurons
        );
        ...
        println!(
            "   _power → A: {} synapses (PSP uniformity: PSP={} per synapse)",
            1000, power_psp
        );
        ...
        println!(
            "   A → B: {} synapses (PSP uniformity: PSP={} per synapse)",
            10000, area_a_psp
        );
        ...
        println!(
            "   B → C: {} synapses (PSP uniformity: PSP={} per synapse)",
            1000, area_b_psp
        );
        ...
        println!(
            "   Rebuilt source_index for {} source neurons",
            synapse_array.source_index.len()
        );
    ...
    fn test_realistic_genome_continuous_simulation() {
        ...
        let x_coords: Vec<u32> = (0..neuron_array.count)
            .map(|i| neuron_array.coordinates[i * 3])
            .collect();
        let y_coords: Vec<u32> = (0..neuron_array.count)
            .map(|i| neuron_array.coordinates[i * 3 + 1])
            .collect();
        let z_coords: Vec<u32> = (0..neuron_array.count)
            .map(|i| neuron_array.coordinates[i * 3 + 2])
            .collect();
        ...
        let source_neurons: Vec<feagi_types::NeuronId> = synapse_array.source_neurons
            [..synapse_array.count]
            .iter()
            .map(|&id| feagi_types::NeuronId(id))
            .collect();
        let target_neurons: Vec<feagi_types::NeuronId> = synapse_array.target_neurons
            [..synapse_array.count]
            .iter()
            .map(|&id| feagi_types::NeuronId(id))
            .collect();
        let weights: Vec<feagi_types::SynapticWeight> = synapse_array.weights
            [..synapse_array.count]
            .iter()
        ...
        for burst_idx in 0..total_bursts {
            ...
            if cpu_result.neuron_count != gpu_result.neuron_count {
                ...
                if mismatches <= 10 {
                    println!(
                        "⚠️  Burst {}: Fire count mismatch! CPU={}, GPU={}",
        ...
        assert_eq!(
            mismatches, 0,
            "CPU and GPU results diverged in {} bursts!",
            mismatches
        );
