...
mod cuda_correctness_tests {
    use feagi_npu_burst_engine::backend::{CPUBackend, CUDABackend, ComputeBackend, is_cuda_available};
    ...
    fn test_synaptic_propagation_correctness() {
        ...
        // Setup CUDA backend
        let mut cuda_backend = CUDABackend::new(100, 200)
            .expect("Failed to create CUDA backend");
        cuda_backend.initialize_persistent_data(&neuron_array, &synapse_array)
        ...
        let cpu_synapse_count = <CPUBackend as ComputeBackend<f32>>::process_synaptic_propagation(&mut cpu_backend,
            &fired_neurons,
            &synapse_array,
            &mut cpu_fcl
        ...
        let gpu_synapse_count = cuda_backend.process_synaptic_propagation(
            &fired_neurons,
            &synapse_array,
            &mut gpu_fcl
        ...
        // Compare results
        println!("CPU: {} synapses processed, {} FCL candidates", cpu_synapse_count, cpu_fcl.len());
        println!("GPU: {} synapses processed, {} FCL candidates", gpu_synapse_count, gpu_fcl.len());
        
        // Verify FCL sizes match
        assert_eq!(cpu_fcl.len(), gpu_fcl.len(), 
            "FCL sizes don't match! CPU: {}, GPU: {}", cpu_fcl.len(), gpu_fcl.len());
        
        // Verify FCL contents match (within floating point tolerance)
        for (cpu_id, cpu_pot) in cpu_fcl.iter() {
            let gpu_pot = gpu_fcl.iter()
                .find(|(id, _)| id == &cpu_id)
                .map(|(_, pot)| pot)
    ...
    fn test_neural_dynamics_correctness() {
        ...
        // CPU execution
        let (cpu_fired, _cpu_fcl_size, _) = <CPUBackend as ComputeBackend<f32>>::process_neural_dynamics(
            &mut cpu_backend,
            &cpu_fcl,
        ...
        // GPU execution
        let (gpu_fired, _gpu_fcl_size, _) = cuda_backend.process_neural_dynamics(
            &gpu_fcl,
            &mut neuron_array,
            1  // burst_count
        ...
        assert_eq!(cpu_fired.len(), gpu_fired.len(),
            "Fired neuron counts don't match! CPU: {}, GPU: {}", 
            cpu_fired.len(), gpu_fired.len());
    ...
    fn test_full_burst_cycle_correctness() {
        ...
        for burst in 1..=5 {
            ...
            // GPU burst cycle
            let mut gpu_fcl = FireCandidateList::new();
            cuda_backend.process_synaptic_propagation(
                &initial_fired,
                &synapse_array,
                &mut gpu_fcl
            ).expect("GPU synaptic propagation failed");
            
            let (gpu_fired, _, _) = cuda_backend.process_neural_dynamics(
                &gpu_fcl,
                &mut neuron_array,
                burst
            ).expect("GPU neural dynamics failed");
    ...
    fn test_large_genome_correctness() {
        ...
        // Setup CUDA backend
        let mut cuda_backend = CUDABackend::new(neuron_count, synapse_count)
            .expect("Failed to create CUDA backend");
        cuda_backend.initialize_persistent_data(&neuron_array, &synapse_array)
            .expect("Failed to initialize CUDA backend");
