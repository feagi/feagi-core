...
fn add_test_synapse(
    ...
    npu.add_synapse(source, target, weight, psp, synapse_type)
        .unwrap();
...
fn test_psp_below_threshold_no_fire() {
    ...
    // Synapse with PSP = 1.0 (weight=1 × psp=1)
    add_test_synapse(
        &mut npu,
        source,
        target,
...
fn test_psp_update_rewrites_existing_outgoing_synapses() {
    ...
    // Start with low PSP (psp=1) so the target will not fire.
    add_test_synapse(
        &mut npu,
        source,
        target,
    ...
    assert!(
        result3.fired_neurons.contains(&source),
        "Source should fire again"
    );
...
fn test_psp_equals_threshold_fires() {
    ...
    // Synapse with PSP = 100 (weight=10 × psp=10)
    add_test_synapse(
        &mut npu,
        source,
        target,
    ...
    npu.inject_sensory_with_potentials(&[(source, 2.0)]);
    let result1 = npu.process_burst().unwrap();
    assert!(result1.fired_neurons.contains(&source));
...
fn test_no_leak_preserves_potential() {
    ...
    assert!(
        (mp - 100.0).abs() < 0.01,
        "With no leak (0.0), potential should be preserved (expected 100, got {})",
        mp
    );
}
...
fn test_partial_leak_decays_potential() {
    ...
    assert!(
        (mp - 50.0).abs() < 0.01,
        "With leak=0.5, potential should decay to 50 (got {})",
        mp
    );
}
...
fn test_full_leak_resets_to_resting() {
    ...
    assert!(
        mp.abs() < 0.01,
        "With leak=1.0, potential should fully decay to resting (0), got {}",
        mp
    );
}
...
fn test_psp_uniformity_false_divides_among_synapses() {
    ...
    assert!(
        result2.fired_neurons.contains(&target1) || result2.fired_neurons.contains(&target2),
...
fn test_psp_uniformity_true_full_to_each_synapse() {
    ...
    assert!(
        result2.fired_neurons.contains(&target1),
        "Target1 should fire with full PSP"
    );
    assert!(
        result2.fired_neurons.contains(&target2),
...
fn test_excitatory_synapse_increases_potential() {
    ...
    assert!(
        result.fired_neurons.contains(&target),
...
fn test_refractory_period_blocks_firing() {
    ...
    assert!(
        result5.fired_neurons.contains(&neuron),
...
fn test_excitability_zero_prevents_firing() {
    ...
    let result = npu.process_burst().unwrap();
    assert!(
        !result.fired_neurons.contains(&neuron),
...
fn test_complex_network_convergence_divergence() {
    ...
    let area_10_id =
        CorticalID::try_from_base_64(&CoreCorticalType::Death.to_cortical_id().as_base_64())
            .unwrap();
    let area_11_id =
        CorticalID::try_from_base_64(&CoreCorticalType::Power.to_cortical_id().as_base_64())
            .unwrap();
    psp_flags.insert(area_10_id, true);
    ...
    let result3 = npu.process_burst().unwrap();
    assert!(
        result3.fired_neurons.contains(&t1),
        "Target 1 should fire from hub"
    );
    assert!(
        result3.fired_neurons.contains(&t2),
