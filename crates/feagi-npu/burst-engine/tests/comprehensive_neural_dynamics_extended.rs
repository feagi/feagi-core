...
fn test_consecutive_fire_count_resets_after_not_firing() {
    ...
    assert!(
        !result3.fired_neurons.contains(&neuron),
        "Blocked by consecutive fire limit"
    );
    ...
    assert!(
        result7.fired_neurons.contains(&neuron),
...
fn test_all_parameters_combined() {
    ...
    let result1 = npu.process_burst().unwrap();
    assert!(
        !result1.fired_neurons.contains(&neuron),
        "Should not fire with 60 < 100"
    );
    ...
    let result2 = npu.process_burst().unwrap();
    assert!(
        result2.fired_neurons.contains(&neuron),
        "Should fire when accumulated potential exceeds threshold"
    );
    ...
    assert!(
        !result4.fired_neurons.contains(&neuron),
        "Should still be in refractory"
    );
    ...
    assert!(
        ...
        "Should fire after refractory expires"
    );
}
