...
use feagi_npu_neural::types::{Error, FireCandidateList, NeuronId, Result};
...
impl CUDABackend {
    ...
    pub fn new_on_device(
        ...
        let device = CudaDevice::new(device_id).map_err(|e| {
            Error::ComputationError(format!("Failed to create CUDA device {}: {}", device_id, e))
        })?;
    ...
    fn initialize_kernels(&mut self) -> Result<()> {
        ...
        let synaptic_ptx =
            include_bytes!(concat!(env!("OUT_DIR"), "/synaptic_propagation_fcl.ptx"));
        ...
        self.synaptic_kernel = Some(
            self.device
                .get_func("synaptic_module", "synaptic_propagation_fcl")
                .ok_or_else(|| {
                    Error::ComputationError("Failed to get synaptic kernel".to_string())
                })?,
        );

        self.neural_kernel = Some(
            self.device
                .get_func("neural_module", "neural_dynamics_fcl")
                .ok_or_else(|| {
                    Error::ComputationError("Failed to get neural kernel".to_string())
                })?,
        );
    ...
    fn upload_neuron_arrays(&mut self, neuron_array: &NeuronArray<f32>) -> Result<()> {
        ...
        if estimated_memory_mb as f64 > self.capabilities.total_memory_gb * 1024.0 * 0.8 {
            warn!(
                "Neuron data ({} MB) may exceed 80% of GPU memory ({} GB)",
        ...
        self.buffers.membrane_potentials = Some(
            self.device
                .htod_copy(neuron_array.membrane_potentials[..count].to_vec())
                .map_err(|e| {
                    Error::ComputationError(format!("Failed to upload membrane potentials: {}", e))
                })?,
        );

        self.buffers.thresholds = Some(
            self.device
                .htod_copy(neuron_array.thresholds[..count].to_vec())
                .map_err(|e| {
                    Error::ComputationError(format!("Failed to upload thresholds: {}", e))
                })?,
        );

        self.buffers.leak_coefficients = Some(
            self.device
                .htod_copy(neuron_array.leak_coefficients[..count].to_vec())
                .map_err(|e| {
                    Error::ComputationError(format!("Failed to upload leak coefficients: {}", e))
                })?,
        );

        self.buffers.resting_potentials = Some(
            self.device
                .htod_copy(neuron_array.resting_potentials[..count].to_vec())
                .map_err(|e| {
                    Error::ComputationError(format!("Failed to upload resting potentials: {}", e))
                })?,
        );

        self.buffers.excitabilities = Some(
            self.device
                .htod_copy(neuron_array.excitabilities[..count].to_vec())
                .map_err(|e| {
                    Error::ComputationError(format!("Failed to upload excitabilities: {}", e))
                })?,
        );

        self.buffers.refractory_countdowns = Some(
            self.device
                .htod_copy(neuron_array.refractory_countdowns[..count].to_vec())
                .map_err(|e| {
                    Error::ComputationError(format!(
                        "Failed to upload refractory countdowns: {}",
                        e
                    ))
                })?,
        );
        ...
        self.buffers.fcl_potentials_atomic =
            Some(self.device.alloc_zeros(fcl_i32_count).map_err(|e| {
                Error::ComputationError(format!("Failed to allocate FCL potentials: {}", e))
            })?);

        self.buffers.fcl_fired_mask =
            Some(self.device.alloc_zeros(fcl_u32_count).map_err(|e| {
                Error::ComputationError(format!("Failed to allocate FCL fired mask: {}", e))
            })?);
    ...
    fn upload_synapse_arrays(&mut self, synapse_array: &SynapseArray) -> Result<()> {
        ...
        self.buffers.synapse_data = Some(self.device.htod_copy(synapse_data).map_err(|e| {
            Error::ComputationError(format!("Failed to upload synapse data: {}", e))
        })?);
        ...
        self.buffers.synapse_hash_keys =
            Some(self.device.htod_copy(hash_keys).map_err(|e| {
                Error::ComputationError(format!("Failed to upload hash keys: {}", e))
            })?);

        self.buffers.synapse_hash_metadata =
            Some(self.device.htod_copy(hash_metadata).map_err(|e| {
                Error::ComputationError(format!("Failed to upload hash metadata: {}", e))
            })?);

        self.buffers.synapse_list = Some(self.device.htod_copy(synapse_list).map_err(|e| {
            Error::ComputationError(format!("Failed to upload synapse list: {}", e))
        })?);

        info!(
            "✅ Uploaded {} synapses ({} MB)",
            synapse_count, required_mb
        );
        Ok(())
    }
    ...
    fn upload_fcl(&mut self, fcl: &FireCandidateList) -> Result<()> {
        ...
        // Upload to GPU
        self.device
            .htod_copy_into(fcl_host, fcl_buffer)
    ...
    fn download_fcl(&self, fcl: &mut FireCandidateList) -> Result<()> {
        ...
        let fcl_host: Vec<i32> = self
            .device
            .dtoh_sync_copy(fcl_buffer)
    ...
    fn download_fired_neurons(&self) -> Result<Vec<u32>> {
        ...
        let mask_host: Vec<u32> = self.device.dtoh_sync_copy(fired_mask).map_err(|e| {
            Error::ComputationError(format!("Failed to download fired mask: {}", e))
        })?;
...
impl ComputeBackend<f32> for CUDABackend {
    ...
    fn process_synaptic_propagation(
        ...
        debug!(
            "🚀 Launching synaptic propagation kernel ({} fired neurons)...",
            fired_neurons.len()
        );

        // Upload fired neurons to GPU
        self.buffers.fired_neurons_staging =
            Some(self.device.htod_copy(fired_neurons.to_vec()).map_err(|e| {
                Error::ComputationError(format!("Failed to upload fired neurons: {}", e))
            })?);
        ...
        unsafe {
            kernel
                ...
                .map_err(|e| {
                    Error::ComputationError(format!("Synaptic kernel launch failed: {}", e))
                })?;
        }
        ...
        self.device.synchronize().map_err(|e| {
            Error::ComputationError(format!(
                "Failed to synchronize after synaptic propagation: {}",
                e
            ))
        })?;
        ...
        debug!(
            "✅ Synaptic propagation complete ({} candidates)",
            fcl.len()
        );
    ...
    fn process_neural_dynamics(
        ...
        debug!(
            "🚀 Launching neural dynamics kernel ({} candidates)...",
            fcl.len()
        );
        ...
        if let Some(fired_mask) = self.buffers.fcl_fired_mask.as_mut() {
            self.device.memset_zeros(fired_mask).map_err(|e| {
                Error::ComputationError(format!("Failed to clear fired mask: {}", e))
            })?;
        }
        ...
        unsafe {
            kernel
                ...
                .map_err(|e| {
                    Error::ComputationError(format!("Neural kernel launch failed: {}", e))
                })?;
        }
        ...
        self.device.synchronize().map_err(|e| {
            Error::ComputationError(format!(
                "Failed to synchronize after neural dynamics: {}",
                e
            ))
        })?;
...
pub fn enumerate_cuda_devices() -> Vec<(usize, String, u64)> {
    ...
    for device_id in 0..16 {
        ...
        if let Ok(device) = CudaDevice::new(device_id) {
            ...
            let caps = CUDABackend::query_capabilities(&device, device_id).unwrap_or_else(|_| {
                GPUCapabilities {
