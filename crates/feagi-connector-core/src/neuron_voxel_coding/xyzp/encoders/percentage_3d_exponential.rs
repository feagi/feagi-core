...
#[derive(Debug)]
pub struct Percentage3DExponentialNeuronVoxelXYZPEncoder {
    channel_dimensions: CorticalChannelDimensions,
    cortical_write_target: CorticalID,
    scratch_space: Vec<(Vec<u32>, Vec<u32>, Vec<u32>)>, // # channels long
}

impl NeuronVoxelXYZPEncoder for Percentage3DExponentialNeuronVoxelXYZPEncoder {
    ...
    fn write_neuron_data_multi_channel_from_processed_cache(&mut self, pipelines: &Vec<SensoryPipelineStageRunner>, time_of_previous_burst: Instant, write_target: &mut CorticalMappedXYZPNeuronVoxels) -> Result<(), FeagiDataError> {
        ...
        pipelines.par_iter()
            ...
            .try_for_each(|(channel_index, (pipeline, scratch))| -> Result<(), FeagiDataError> {
                ...
                encode_unsigned_percentage_to_fractional_exponential_neuron_z_indexes(&updated_percentage_3d.a, self.channel_dimensions.depth, &mut scratch.0);
                encode_unsigned_percentage_to_fractional_exponential_neuron_z_indexes(&updated_percentage_3d.b, self.channel_dimensions.depth, &mut scratch.1);
                encode_unsigned_percentage_to_fractional_exponential_neuron_z_indexes(&updated_percentage_3d.c, self.channel_dimensions.depth, &mut scratch.2);
