...
#[derive(Debug)]
pub struct GazePropertiesExponentialNeuronVoxelXYZPDecoder {
    ...
    fn read_neuron_data_multi_channel_into_pipeline_input_cache(&mut self, neurons_to_read: &CorticalMappedXYZPNeuronVoxels, time_of_read: Instant, pipelines_with_data_to_update: &mut Vec<MotorPipelineStageRunner>, channel_changed: &mut Vec<bool>) -> Result<(), FeagiDataError> {
        ...
        for channel_index in 0..number_of_channels as usize { // Literally not worth making parallel... right?
            ...
            if !eccentricity_z_a_vector.is_empty() {
                decode_unsigned_percentage_from_fractional_exponential_neurons(&eccentricity_z_a_vector, &mut prev_gaze.eccentricity_location_xy.a);
            }

            if !eccentricity_z_b_vector.is_empty() {
                decode_unsigned_percentage_from_fractional_exponential_neurons(&eccentricity_z_b_vector, &mut prev_gaze.eccentricity_location_xy.b);
            }

            if !modularity_z_vector.is_empty() {
                decode_unsigned_percentage_from_fractional_exponential_neurons(&modularity_z_vector, &mut prev_gaze.modulation_size);
            }
...
impl GazePropertiesExponentialNeuronVoxelXYZPDecoder {
    pub fn new_box(eccentricity_cortical_id: CorticalID, modularity_cortical_id: CorticalID, eccentricity_z_depth: NeuronDepth, modularity_z_depth: NeuronDepth, number_channels: CorticalChannelCount) -> Result<Box<dyn NeuronVoxelXYZPDecoder + Sync + Send>, FeagiDataError> {
        ...
        let decoder = GazePropertiesExponentialNeuronVoxelXYZPDecoder {
