...
#[derive(Debug)]
pub struct SignedPercentageExponentialNeuronVoxelXYZPDecoder {
...
impl NeuronVoxelXYZPDecoder for SignedPercentageExponentialNeuronVoxelXYZPDecoder {
    ...
    fn read_neuron_data_multi_channel_into_pipeline_input_cache(&mut self, neurons_to_read: &CorticalMappedXYZPNeuronVoxels, _time_of_read: Instant, pipelines_with_data_to_update: &mut Vec<MotorPipelineStageRunner>, channel_changed: &mut Vec<bool>) -> Result<(), FeagiDataError> {
        ...
        for channel_index in 0..number_of_channels as usize { // Literally not worth making parallel... right?
            ...
            let z_row_vector_positive = self.z_depth_scratch_space_positive.get(z_row_index).unwrap();
            let z_row_vector_negative = self.z_depth_scratch_space_negative.get(z_row_index).unwrap();
            ...
            let signed_percentage: &mut SignedPercentage = pipelines_with_data_to_update.get_mut(channel_index).unwrap().get_preprocessed_cached_value_mut().try_into()?;

            decode_signed_percentage_from_fractional_exponential_neurons(&z_row_vector_positive, &z_row_vector_negative, signed_percentage);
        };
...
impl SignedPercentageExponentialNeuronVoxelXYZPDecoder {
    ...
    pub fn new_box(cortical_read_target: CorticalID, z_resolution: NeuronDepth, number_channels: CorticalChannelCount) -> Result<Box<dyn NeuronVoxelXYZPDecoder + Sync + Send>, FeagiDataError> {
        ...
        let decoder = SignedPercentageExponentialNeuronVoxelXYZPDecoder {
