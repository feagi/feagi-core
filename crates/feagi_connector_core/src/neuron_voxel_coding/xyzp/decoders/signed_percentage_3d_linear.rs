...
#[derive(Debug)]
pub struct SignedPercentage3DLinearNeuronVoxelXYZPDecoder {
...
impl NeuronVoxelXYZPDecoder for SignedPercentage3DLinearNeuronVoxelXYZPDecoder {
    ...
    fn read_neuron_data_multi_channel_into_pipeline_input_cache(&mut self, neurons_to_read: &CorticalMappedXYZPNeuronVoxels, _time_of_read: Instant, pipelines_with_data_to_update: &mut Vec<MotorPipelineStageRunner>, channel_changed: &mut Vec<bool>) -> Result<(), FeagiDataError> {
        ...
        for channel_index in 0..number_of_channels as usize { // Literally not worth making parallel... right?
            ...
            if z_a_row_vector_positive.is_empty() && z_b_row_vector_positive.is_empty() && z_c_row_vector_positive.is_empty() 
                && z_a_row_vector_negative.is_empty() && z_b_row_vector_negative.is_empty() && z_c_row_vector_negative.is_empty() {
                continue; // No data collected for this channel. Do not emit
            }
            channel_changed[channel_index] = true;
            let signed_percentage_3d: &mut SignedPercentage3D = pipelines_with_data_to_update.get_mut(channel_index).unwrap().get_preprocessed_cached_value_mut().try_into()?;

            if !(z_a_row_vector_positive.is_empty() && z_a_row_vector_negative.is_empty()) {
                decode_signed_percentage_from_linear_neurons(&z_a_row_vector_positive, &z_a_row_vector_negative, self.channel_dimensions.depth, &mut signed_percentage_3d.a);
            }
            if !(z_b_row_vector_positive.is_empty() && z_b_row_vector_negative.is_empty()) {
                decode_signed_percentage_from_linear_neurons(&z_b_row_vector_positive, &z_b_row_vector_negative, self.channel_dimensions.depth, &mut signed_percentage_3d.b);
            }
            if !(z_c_row_vector_positive.is_empty() && z_c_row_vector_negative.is_empty()) {
                decode_signed_percentage_from_linear_neurons(&z_c_row_vector_positive, &z_c_row_vector_negative, self.channel_dimensions.depth, &mut signed_percentage_3d.c);
            }
        };
...
impl SignedPercentage3DLinearNeuronVoxelXYZPDecoder {
    ...
    pub fn new_box(cortical_read_target: CorticalID, z_resolution: NeuronDepth, number_channels: CorticalChannelCount) -> Result<Box<dyn NeuronVoxelXYZPDecoder + Sync + Send>, FeagiDataError> {
        ...
        let decoder = SignedPercentage3DLinearNeuronVoxelXYZPDecoder {
