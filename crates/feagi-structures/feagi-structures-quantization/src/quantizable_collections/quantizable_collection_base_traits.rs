use crate::FeagiQuantizedHardwareError;
...
QuantizableCollectionBaseTrait<QuantIndex, WrappedIndexCountType, Value>
{
    ...
    fn get_value_at_linear_index_checked(&self, linear_index: WrappedIndexCountType) -> Result<&Value, FeagiQuantizedHardwareError> {
        if ! self.does_value_exist_at_linear_index(linear_index) {
            return Err(FeagiQuantizedHardwareError::CollectionInvalidIndexError {
    ...
    fn get_value_at_linear_index_checked_mut(&mut self, linear_index: WrappedIndexCountType) -> Result<&mut Value, FeagiQuantizedHardwareError> {
        if ! self.does_value_exist_at_linear_index(linear_index) {
            return Err(FeagiQuantizedHardwareError::CollectionInvalidIndexError {
                context: Self::INVALID_LINEAR_INDEX_STR,
                invalid_index: linear_index.quant_ref().to_u32(),
