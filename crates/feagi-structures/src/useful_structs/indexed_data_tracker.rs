...
#[derive(Debug, Clone)]
pub struct IndexedDataTracker<DataType, IndexType: QuantizedIndexCountTrait> {
    data: Vec<Option<DataType>>,
    skipped_indexes: Vec<IndexType>,
    index_length: IndexType,
}

impl<DataType, IndexType: QuantizedIndexCountTrait> IndexedDataTracker<DataType, IndexType> {
    ...
    pub fn return_index_and_invalidate_data(&mut self, returning_index: IndexType) -> Result<(), FeagiCommonError> {
...
impl<T, IndexType: QuantizedIndexCountTrait> Default for IndexedDataTracker<T, IndexType> {
    fn default() -> Self {
        Self::new()
    }
}

impl<T, IndexType: QuantizedIndexCountTrait> Index<IndexType> for IndexedDataTracker<T, IndexType> {
    type Output = T;
...
impl<T, IndexType: QuantizedIndexCountTrait> IndexMut<IndexType> for  IndexedDataTracker<T, IndexType> {
