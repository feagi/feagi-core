...
use crate::neuron::dimensional::dimensional_structs::{NeuronVoxelCoordinate, NeuronVoxelDimensions, NeuronVoxelIndexCount, NeuronVoxelPotential};
use crate::quantization_level::CorticalAreaNeuronQuantization;

/// Neuron Voxel iteration item. Since voxels will only show neuron potential,
/// we will only include that as a value
pub struct NeuronVoxelIterItem<'a, CANQ: CorticalAreaNeuronQuantization>
{
...
impl<'a, CANQ: CorticalAreaNeuronQuantization> NeuronVoxelIterItem<'a, CANQ> {
...
pub enum DimensionalNeuronVoxelSmartIterator<'a, CANQ, SDI, MDI>
where
...
impl<'a, CANQ, SDI, MDI> Iterator for DimensionalNeuronVoxelSmartIterator<'a, CANQ, SDI, MDI>
where
    CANQ: CorticalAreaNeuronQuantization,
    //SDI: SingleDensityIterator<Item = EDMN>,
    //MDI: MultipleDensityIterator<Item = EDMN>,
{
    type Item = NeuronVoxelIterItem<'a, CANQ>;
    fn next(&mut self) -> Option<Self::Item> {
        match self {
            DimensionalNeuronVoxelSmartIterator::Single(iter) => iter.next(),
            DimensionalNeuronVoxelSmartIterator::Multiple(iter) => iter.next(),
        }
    }
}

impl<'a, CANQ, SDI, MDI> DimensionalNeuronVoxelSmartIterator<'a, CANQ, SDI, MDI>
where
    CANQ: CorticalAreaNeuronQuantization,
    //SDI: SingleDensityIterator<Item = DMN>,
    //MDI: MultipleDensityIterator<Item = DMN>,
{
    pub fn iter_skip_zero_potential(self) -> impl Iterator<Item = NeuronVoxelIterItem<'a, CANQ>>
    {
