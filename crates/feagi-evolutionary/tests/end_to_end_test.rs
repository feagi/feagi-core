...
fn test_complete_genome_workflow() {
    ...
    assert!(
        validation2.errors.is_empty(),
        "Loaded genome should have no errors: {:?}",
        validation2.errors
    );
    ...
    println!(
        "   - Created genome with {} morphologies",
        loaded_genome.morphologies.count()
    );
    println!(
        "   - Cortical areas: {}",
        loaded_genome.cortical_areas.len()
    );
