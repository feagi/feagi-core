...
macro_rules! __impl_quantized_element_wrapper_base {
    ($wrapper_type:ident, $quant_element:ident, $($quant_bound:tt)+) => {
        impl<$quant_element> $crate::data_wrappers::quantizable::wrapper_traits::QuantizedElementWrapperBase<$quant_element> for $wrapper_type<$quant_element>
        where
            $quant_element: $($quant_bound)+,
        {
            
            const QUANT_ZERO: Self =
                Self(<$quant_element as $crate::feagi_common_quantizable::shared_traits::QuantizedElementBase>::QUANT_ZERO);
...
macro_rules! __impl_quantized_element_wrapper_base_concrete {
    ($wrapper_type:ident, $quant_element:ty, $($quant_bound:tt)+) => {
        impl $crate::data_wrappers::quantizable::wrapper_traits::QuantizedElementWrapperBase<$quant_element> for $wrapper_type
        where
            $quant_element: $($quant_bound)+,
        {
...
macro_rules! __impl_quantized_element_base {
    ($wrapper_type:ident, $quant_element:ident, $($quant_bound:tt)+) => {
        impl<$quant_element> $crate::feagi_common_quantizable::shared_traits::QuantizedElementBase for $wrapper_type<$quant_element>
        where
            $quant_element: $($quant_bound)+,
        {
            const QUANTIZATION_LEVEL: $crate::feagi_common_quantizable::QuantizationLevel =
                <$quant_element as $crate::feagi_common_quantizable::shared_traits::QuantizedElementBase>::QUANTIZATION_LEVEL;
            const QUANT_ZERO: Self =
                Self(<$quant_element as $crate::feagi_common_quantizable::shared_traits::QuantizedElementBase>::QUANT_ZERO);
        }
    };
...
macro_rules! __impl_quantized_element_base_concrete {
    ($wrapper_type:ident, $quant_element:ty, $($quant_bound:tt)+) => {
        impl $crate::feagi_common_quantizable::shared_traits::QuantizedElementBase for $wrapper_type
        where
            $quant_element: $($quant_bound)+,
        {
            const QUANTIZATION_LEVEL: $crate::feagi_common_quantizable::QuantizationLevel =
...
macro_rules! __impl_supports_basic_core_math_ops {
    ($wrapper_type:ident, $quant_element:ident, $($quant_bound:tt)+) => {
        ...
        impl<$quant_element> $crate::feagi_common_quantizable::shared_traits::SupportsBasicCoreMathOps for $wrapper_type<$quant_element>
        where
            $quant_element: $($quant_bound)+,
        {
        }
...
macro_rules! __impl_supports_basic_core_math_ops_concrete {
    ($wrapper_type:ident, $quant_element:ty, $($quant_bound:tt)+) => {
        ...
        impl $crate::feagi_common_quantizable::shared_traits::SupportsBasicCoreMathOps for $wrapper_type
        where
            $quant_element: $($quant_bound)+,
        {
        }
...
macro_rules! __impl_supports_uint_ops {
    ($wrapper_type:ident, $quant_element:ident, $($quant_bound:tt)+) => {
        ...
        impl<$quant_element> $crate::feagi_common_quantizable::shared_traits::SupportsUintOps for $wrapper_type<$quant_element>
        where
            $quant_element: $($quant_bound)+,
        {
            const QUANT_MAX: Self =
                Self(<$quant_element as $crate::feagi_common_quantizable::shared_traits::SupportsUintOps>::QUANT_MAX);

            const QUANT_MAX_AS_USIZE: usize =
                <$quant_element as $crate::feagi_common_quantizable::shared_traits::SupportsUintOps>::QUANT_MAX_AS_USIZE;
            const QUANT_ONE: Self =
                Self(<$quant_element as $crate::feagi_common_quantizable::shared_traits::SupportsUintOps>::QUANT_ONE);

            #[inline(always)]
            fn from_usize_unchecked(u: usize) -> Self {
                Self(<$quant_element as $crate::feagi_common_quantizable::shared_traits::SupportsUintOps>::from_usize_unchecked(u))
            }

            #[inline(always)]
            fn from_usize_clamped(u: usize) -> Self {
                Self(<$quant_element as $crate::feagi_common_quantizable::shared_traits::SupportsUintOps>::from_usize_clamped(u))
            }

            #[inline(always)]
            fn to_usize(self) -> usize {
                <$quant_element as $crate::feagi_common_quantizable::shared_traits::SupportsUintOps>::to_usize(self.0)
            }
        }
    };
}
...
macro_rules! __impl_supports_uint_ops_concrete {
    ($wrapper_type:ident, $quant_element:ty, $($quant_bound:tt)+) => {
        ...
        impl $crate::feagi_common_quantizable::shared_traits::SupportsUintOps for $wrapper_type
        where
            $quant_element: $($quant_bound)+,
        {
            const QUANT_MAX: Self =
