...
pub trait BitPacked<QI: QuantizedUnsignedIntegerTrait>: Index<QI, Output = u32> + Index<Range<QI>, Output = [u32]> {
    ...
    fn u32_count_for_bits(number_elements: QI, byte_alignment: usize) -> usize {
        ...
        number_elements.quant_to_usize().div_ceil(32).div_ceil(u32s_per_alignment) * u32s_per_alignment
    }
    ...
    /// Returns `true` if there are no u32s.
    fn is_empty(&self) -> bool {
        self.as_u32s().len() == 0
    }

    /// Copies out the bit at `index`, assumes the given index is within the non-padded area
    fn get_bit_unchecked(&self, bit_index: QI) -> bool {
        let bit = bit_index.quant_to_usize();
        let byte = self.as_u32s()[bit / 32];
        // '& 0b00011111' is the same as  '% 32'
        (byte >> (bit & 0b00011111)) & 1 == 1
    }
...
pub trait BitPackedAwareSize<QI: QuantizedUnsignedIntegerTrait>: BitPacked<QI> {
    ...
    /// Borrows the whole collection as a [`BitPackedSliceSizeAware`] view.
    fn as_bit_packed_slice(&self) -> BitPackedSliceSizeAware<'_, QI> {
        BitPackedSliceSizeAware::new(self.as_u32s(), self.number_addressable_bits())
    }

    /// Borrows a half-open *u32-word* sub-range as a [`BitPackedSliceSizeAware`] view. The
    ...
    fn subslice(&self, range: Range<QI>) -> Result<BitPackedSliceSizeAware<'_, QI>, BitPackError> {
        ...
        match self.as_u32s().get(start..end) {
            Some(slice) => {
                ...
                Ok(BitPackedSliceSizeAware::new(slice, bits))
            }
            None => Err(BitPackInvalidRange::new("subslice u32 range is out of bounds", start, end).into()),
        }
    }
...
/// or exclusively borrow their storage (the [`BitPackedVectorSizeAware`],
/// [`BitPackedSliceMutSizeAware`] view, and [`BitPackedArraySizeAware`]).
///
/// Implementors only need to expose their backing storage via
/// [`Self::as_mut_bytes`].
pub trait BitPackedMut<QI: QuantizedUnsignedIntegerTrait>: BitPacked<QI> + IndexMut<QI, Output = u32> + IndexMut<Range<QI>, Output = [u32]> {
...
pub trait BitPackedAwareSizeMut<QI: QuantizedUnsignedIntegerTrait>: BitPackedMut<QI> + BitPackedAwareSize<QI> {
    //region Default impls
    ...
    /// Mutably borrows a half-open *u32-word* sub-range as a [`BitPackedSliceMutSizeAware`]
    fn subslice_u32s_mut(&mut self, range: Range<QI>) -> Result<BitPackedSliceMutSizeAware<'_, QI>, BitPackError> {
        let start = range.start.quant_to_usize();
        let end = range.end.quant_to_usize();
        match self.as_mut_u32s().get_mut(start..end) {
            Some(slice) => {
                let bits = QI::quant_from_usize_unchecked(slice.len() * 32);
                Ok(BitPackedSliceMutSizeAware::new(slice, bits))
            }
            None => Err(BitPackInvalidRange::new("subslice u32 range is out of bounds", start, end).into()),
        }
    }
...
/// Behaviour for bit-packed collections that do not track how many bits are
/// logically in use. Methods that depend on the logical bit count take
/// `number_addressable_bits` as an external parameter.
pub trait BitPackedUnawareSize<QI: QuantizedUnsignedIntegerTrait>: BitPacked<QI> {
    //region Default impls

    /// Number of unused ("dangling") bits in the final u32 word, i.e. the bits of
    /// the backing storage beyond `number_addressable_bits`.
    fn number_dangling_bits(&self, number_addressable_bits: QI) -> u32 {
        let capacity = self.as_u32s().len() * 32;
        (capacity - number_addressable_bits.quant_to_usize()) as u32
    }

    /// Returns `true` if there are no addressable bits.
    fn is_empty(&self, number_addressable_bits: QI) -> bool {
        number_addressable_bits == QI::QUANT_ZERO
    }

    /// Copies out the bit at `index`, or `None` if out of bounds / in padding.
    fn get_bit(&self, bit_index: QI, number_addressable_bits: QI) -> Option<bool> {
        if bit_index >= number_addressable_bits {
            return None;
        }
        Some(self.get_bit_unchecked(bit_index))
    }

    /// Borrows the whole collection as a [`BitPackedSliceSizeAware`] view using
    /// the supplied logical bit count.
    fn as_bit_packed_slice(&self, number_addressable_bits: QI) -> BitPackedSliceSizeAware<'_, QI> {
        BitPackedSliceSizeAware::new(self.as_u32s(), number_addressable_bits)
    }
    ...
    /// so any dangling bits of the original collection are not carried over.
    ///
    /// Returns [`BitPackInvalidRange`] if `range` is out of bounds or its
    /// start is greater than its end (rather than panicking like `self[range]`).
    fn subslice(&self, range: Range<QI>) -> Result<BitPackedSliceSizeAware<'_, QI>, BitPackError> {
        let start = range.start.quant_to_usize();
        let end = range.end.quant_to_usize();
    ...
    /// eventually the last u32 word will cover a range less than 32. This function
    /// will return less than 32 if that's the case.
    fn get_number_bits_remaining_from_u32_unchecked(&self, u32_index: QI, number_addressable_bits: QI) -> u32 {
        let first_index = self.get_first_bit_index_from_u32_unchecked(u32_index);
        let bits_remaining = (number_addressable_bits - first_index).quant_to_usize();
        bits_remaining.min(32) as u32
    }
...
/// Mutable behaviour for bit-packed collections that do not track how many bits
/// are logically in use. Methods that depend on the logical bit count take
/// `number_addressable_bits` as an external parameter.
pub trait BitPackedUnawareSizeMut<QI: QuantizedUnsignedIntegerTrait>: BitPackedMut<QI> + BitPackedUnawareSize<QI> {
    //region Default impls

    /// Sets the bit at `index`, returning the previous value if the index was in
    /// bounds (otherwise leaves the collection untouched).
    fn set_bit(&mut self, index: QI, value: bool, number_addressable_bits: QI) -> Option<bool> {
        let bit = index.quant_to_usize();
        if bit >= number_addressable_bits.quant_to_usize() {
            return None;
        }
        let word = &mut self.as_mut_u32s()[bit >> 5];
        let previous = (*word >> (bit & 0x1F)) & 1 == 1;
...
//region Size Unaware Implementations
//
// These collections do not track a logical bit count. Every u32 word in the
// backing storage is treated as fully utilised (`data.len() * 32` addressable bits).
...
impl<QI: QuantizedUnsignedIntegerTrait> BitPackedVectorSizeUnaware<QI> {
    ...
    /// to `initial_state`.
    pub fn new_uniform(number_u32s: QI, initial_state: bool) -> BitPackedVectorSizeUnaware<QI> {
    ...
    /// Builds a vector large enough to hold `number_bits`, aligned to
    /// [`MIN_U32_BYTE_ALIGNMENT`]. Every bit in the allocated storage is treated
    /// as addressable (including any padding bits in the final u32 word).
    pub fn new_uniform_for_bits(number_bits: QI, initial_state: bool) -> BitPackedVectorSizeUnaware<QI> {
        let number_u32s = <Self as BitPacked<QI>>::u32_count_for_bits(number_bits, MIN_U32_BYTE_ALIGNMENT);
        Self::new_uniform(QI::quant_from_usize_unchecked(number_u32s), initial_state)
    }

    /// Wraps an existing `Vec` without copying. Every u32 is treated as full
    /// (`data.len() * 32` addressable bits).
    pub fn from_vec(data: Vec<u32>) -> BitPackedVectorSizeUnaware<QI> {
        Self {
            data,
            _marker: core::marker::PhantomData,
        }
    ...
    /// Appends `number_u32s` new u32 words to the end of the vector, initializing
    /// every bit in each appended word to `initial_state`.
    pub fn append_u32s(&mut self, number_u32s: QI, initial_state: bool) {
        let additional_u32s = number_u32s.quant_to_usize();
        if additional_u32s == 0 {
            return;
        }

        let fill = if initial_state { 0xFFFF_FFFF } else { 0 };
        self.data.extend(core::iter::repeat_n(fill, additional_u32s));
    }
}
...
impl<'a, QI: QuantizedUnsignedIntegerTrait> BitPackedSliceSizeUnaware<'a, QI> {
    /// Wraps an existing shared u32 slice. Every u32 is treated as full
    /// (`data.len() * 32` addressable bits).
    pub fn new(data: &'a [u32]) -> BitPackedSliceSizeUnaware<'a, QI> {
        Self {
            data,
            _marker: core::marker::PhantomData,
        }
    ...
    /// Alias for [`Self::new`].
    pub fn from_u32s(data: &'a [u32]) -> BitPackedSliceSizeUnaware<'a, QI> {
        Self::new(data)
    }
...
impl<'a, QI: QuantizedUnsignedIntegerTrait> BitPacked<QI> for BitPackedSliceSizeUnaware<'a, QI> {
    fn as_u32s(&self) -> &[u32] {
        self.data
    }
}

impl<'a, QI: QuantizedUnsignedIntegerTrait> BitPackedUnawareSize<QI> for BitPackedSliceSizeUnaware<'a, QI> {}
...
impl_bitpacked_range_read!(
    BitPackedSliceSizeUnaware<'a, QI>,
    QI,
    ['a, QI: QuantizedUnsignedIntegerTrait]
);

impl<'a, QI: QuantizedUnsignedIntegerTrait> From<&'a [u32]> for BitPackedSliceSizeUnaware<'a, QI> {
...
impl<'a, QI: QuantizedUnsignedIntegerTrait> BitPackedSliceMutSizeUnaware<'a, QI> {
    /// Wraps an existing mutable u32 slice. Every u32 is treated as full
    /// (`data.len() * 32` addressable bits).
    pub fn new(data: &'a mut [u32]) -> BitPackedSliceMutSizeUnaware<'a, QI> {
        Self {
            data,
            _marker: core::marker::PhantomData,
        }
    ...
    /// Alias for [`Self::new`].
    pub fn from_u32s(data: &'a mut [u32]) -> BitPackedSliceMutSizeUnaware<'a, QI> {
        Self::new(data)
    }
    ...
    pub fn reborrow(&mut self) -> BitPackedSliceMutSizeUnaware<'_, QI> {
        BitPackedSliceMutSizeUnaware::new(self.data)
    }
}

impl<'a, QI: QuantizedUnsignedIntegerTrait> BitPacked<QI> for BitPackedSliceMutSizeUnaware<'a, QI> {
    fn as_u32s(&self) -> &[u32] {
        self.data
    }
}

impl<'a, QI: QuantizedUnsignedIntegerTrait> BitPackedMut<QI> for BitPackedSliceMutSizeUnaware<'a, QI> {
    fn as_mut_u32s(&mut self) -> &mut [u32] {
        self.data
    }
}

impl<'a, QI: QuantizedUnsignedIntegerTrait> BitPackedUnawareSize<QI> for BitPackedSliceMutSizeUnaware<'a, QI> {}

impl<'a, QI: QuantizedUnsignedIntegerTrait> BitPackedUnawareSizeMut<QI> for BitPackedSliceMutSizeUnaware<'a, QI> {}
...
impl<'a, QI: QuantizedUnsignedIntegerTrait> IndexMut<QI> for BitPackedSliceMutSizeUnaware<'a, QI> {
    fn index_mut(&mut self, index: QI) -> &mut Self::Output {
        &mut self.data[index.quant_to_usize()]
    }
}
...
impl_bitpacked_range_read_write!(
    BitPackedSliceMutSizeUnaware<'a, QI>,
    QI,
    ['a, QI: QuantizedUnsignedIntegerTrait]
);

impl<'a, QI: QuantizedUnsignedIntegerTrait> From<&'a mut [u32]> for BitPackedSliceMutSizeUnaware<'a, QI> {
...
/// An owned, stack-allocated run of u32 bit-packed booleans backed by exactly
/// `N` u32 words, all fully utilised (`N * 32` addressable bits).
///
...
impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> BitPackedArraySizeUnaware<QI, N> {
    /// Builds an array of `N` u32 words, every bit in every word initialised to
    /// `initial_state`.
    pub fn new_uniform(initial_state: bool) -> BitPackedArraySizeUnaware<QI, N> {
        let data: [u32; N] = if initial_state { [0xFFFF_FFFF; N] } else { [0; N] };
    ...
    /// Wraps an existing array. Every u32 is treated as full (`N * 32` addressable bits).
    pub fn from_array(data: [u32; N]) -> BitPackedArraySizeUnaware<QI, N> {
        Self {
            data,
            _marker: core::marker::PhantomData,
        }
...
impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> BitPacked<QI> for BitPackedArraySizeUnaware<QI, N> {
    fn as_u32s(&self) -> &[u32] {
        &self.data
    }
}

impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> BitPackedMut<QI> for BitPackedArraySizeUnaware<QI, N> {
    fn as_mut_u32s(&mut self) -> &mut [u32] {
        &mut self.data
    }
}

impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> BitPackedUnawareSize<QI> for BitPackedArraySizeUnaware<QI, N> {}

impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> BitPackedUnawareSizeMut<QI> for BitPackedArraySizeUnaware<QI, N> {}
...
impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> IndexMut<QI> for BitPackedArraySizeUnaware<QI, N> {
    fn index_mut(&mut self, index: QI) -> &mut Self::Output {
        &mut self.data[index.quant_to_usize()]
    }
}
...
impl_bitpacked_range_read_write!(
    BitPackedArraySizeUnaware<QI, N>,
    QI,
    [QI: QuantizedUnsignedIntegerTrait, const N: usize]
);

impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> From<[u32; N]> for BitPackedArraySizeUnaware<QI, N> {
...
//region Size Aware Implementations
...
impl<QI: QuantizedUnsignedIntegerTrait> BitPackedVectorSizeAware<QI> {
    ...
    pub fn new_uniform(number_bits: QI, initial_state: bool) -> BitPackedVectorSizeAware<QI> {
        let number_u32s = QI::quant_from_usize_unchecked(<Self as BitPacked<QI>>::u32_count_for_bits(number_bits, MIN_U32_BYTE_ALIGNMENT));
        let data: Vec<u32> = if initial_state {
            vec![0xFFFF_FFFF; number_u32s.quant_to_usize()]
        } else {
            vec![0; number_u32s.quant_to_usize()]
    ...
    pub fn from_vec(data: Vec<u32>) -> BitPackedVectorSizeAware<QI> {
    ...
    pub fn from_vec_with_bits(data: Vec<u32>, number_bits: QI) -> BitPackedVectorSizeAware<QI> {
        Self { data, number_bits }
    }
    ...
    pub fn append_bits(&mut self, number_bits: QI, initial_state: bool) {
        ...
        let needed_u32s = <Self as BitPacked<QI>>::u32_count_for_bits(QI::quant_from_usize_unchecked(new_total_bits), MIN_U32_BYTE_ALIGNMENT);
...
impl<'a, QI: QuantizedUnsignedIntegerTrait> BitPackedSliceSizeAware<'a, QI> {
    /// Wraps an existing shared u32 slice with an explicit bit count.
    /// `number_bits` must not exceed `data.len() * 32`.
    pub fn new(data: &'a [u32], number_bits: QI) -> BitPackedSliceSizeAware<'a, QI> {
        Self { data, number_bits }
    }

    /// Wraps an existing shared u32 slice, treating every u32 as full
    /// (bit count is `data.len() * 32`, no dangling bits).
    pub fn from_u32s(data: &'a [u32]) -> BitPackedSliceSizeAware<'a, QI> {
        let number_bits = QI::quant_from_usize_unchecked(data.len() * 32);
        Self { data, number_bits }
    }
...
impl<'a, QI: QuantizedUnsignedIntegerTrait> BitPacked<QI> for BitPackedSliceSizeAware<'a, QI> {
    fn as_u32s(&self) -> &[u32] {
        self.data
    }
}

impl<'a, QI: QuantizedUnsignedIntegerTrait> BitPackedAwareSize<QI> for BitPackedSliceSizeAware<'a, QI> {
    fn number_addressable_bits(&self) -> QI {
        self.number_bits
    }
}

impl<'a, QI: QuantizedUnsignedIntegerTrait> Index<QI> for BitPackedSliceSizeAware<'a, QI> {
    type Output = u32;
    fn index(&self, index: QI) -> &Self::Output {
        &self.data[index.quant_to_usize()]
    }
...
impl_bitpacked_range_read!(
    BitPackedSliceSizeAware<'a, QI>,
    QI,
    ['a, QI: QuantizedUnsignedIntegerTrait]
);

impl<'a, QI: QuantizedUnsignedIntegerTrait> From<&'a [u32]> for BitPackedSliceSizeAware<'a, QI> {
    fn from(value: &'a [u32]) -> Self {
        Self::from_u32s(value)
    }
}
...
impl<'a, QI: QuantizedUnsignedIntegerTrait> BitPackedSliceMutSizeAware<'a, QI> {
    /// Wraps an existing mutable u32 slice with an explicit bit count.
    /// `number_bits` must not exceed `data.len() * 32`.
    pub fn new(data: &'a mut [u32], number_bits: QI) -> BitPackedSliceMutSizeAware<'a, QI> {
        Self { data, number_bits }
    }

    /// Wraps an existing mutable u32 slice, treating every u32 as full
    /// (bit count is `data.len() * 32`, no dangling bits).
    pub fn from_u32s(data: &'a mut [u32]) -> BitPackedSliceMutSizeAware<'a, QI> {
        let number_bits = QI::quant_from_usize_unchecked(data.len() * 32);
        Self { data, number_bits }
    }
    ...
    /// Creates a shorter-lived, exclusive re-borrow of this view.
    pub fn reborrow(&mut self) -> BitPackedSliceMutSizeAware<'_, QI> {
        BitPackedSliceMutSizeAware::new(self.data, self.number_bits)
    }
}

impl<'a, QI: QuantizedUnsignedIntegerTrait> BitPacked<QI> for BitPackedSliceMutSizeAware<'a, QI> {
    fn as_u32s(&self) -> &[u32] {
        self.data
    }
}

impl<'a, QI: QuantizedUnsignedIntegerTrait> BitPackedAwareSize<QI> for BitPackedSliceMutSizeAware<'a, QI> {
    fn number_addressable_bits(&self) -> QI {
        self.number_bits
    }
}

impl<'a, QI: QuantizedUnsignedIntegerTrait> BitPackedMut<QI> for BitPackedSliceMutSizeAware<'a, QI> {
    fn as_mut_u32s(&mut self) -> &mut [u32] {
        self.data
    }
}

impl<'a, QI: QuantizedUnsignedIntegerTrait> BitPackedAwareSizeMut<QI> for BitPackedSliceMutSizeAware<'a, QI> {}

impl<'a, QI: QuantizedUnsignedIntegerTrait> Index<QI> for BitPackedSliceMutSizeAware<'a, QI> {
    type Output = u32;
    fn index(&self, index: QI) -> &Self::Output {
        &self.data[index.quant_to_usize()]
    }
...
impl<'a, QI: QuantizedUnsignedIntegerTrait> IndexMut<QI> for BitPackedSliceMutSizeAware<'a, QI> {
    fn index_mut(&mut self, index: QI) -> &mut Self::Output {
        &mut self.data[index.quant_to_usize()]
    }
}
...
impl_bitpacked_range_read_write!(
    BitPackedSliceMutSizeAware<'a, QI>,
    QI,
    ['a, QI: QuantizedUnsignedIntegerTrait]
);

impl<'a, QI: QuantizedUnsignedIntegerTrait> From<&'a mut [u32]> for BitPackedSliceMutSizeAware<'a, QI> {
    fn from(value: &'a mut [u32]) -> Self {
        Self::from_u32s(value)
    }
}
...
impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> BitPackedArraySizeAware<QI, N> {
    ...
    pub fn new_uniform(number_bits: QI, initial_state: bool) -> BitPackedArraySizeAware<QI, N> {
        let data: [u32; N] = if initial_state { [0xFFFF_FFFF; N] } else { [0; N] };
    ...
    pub fn from_array(data: [u32; N]) -> BitPackedArraySizeAware<QI, N> {
    ...
    /// exceed `N * 32`.
    pub fn from_array_with_bits(data: [u32; N], number_bits: QI) -> BitPackedArraySizeAware<QI, N> {
        Self { data, number_bits }
    }
...
impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> BitPacked<QI> for BitPackedArraySizeAware<QI, N> {
    fn as_u32s(&self) -> &[u32] {
        &self.data
    }
}

impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> BitPackedAwareSize<QI> for BitPackedArraySizeAware<QI, N> {
    fn number_addressable_bits(&self) -> QI {
        self.number_bits
    }
}

impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> BitPackedMut<QI> for BitPackedArraySizeAware<QI, N> {
    fn as_mut_u32s(&mut self) -> &mut [u32] {
        &mut self.data
    }
}

impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> BitPackedAwareSizeMut<QI> for BitPackedArraySizeAware<QI, N> {}
...
impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> IndexMut<QI> for BitPackedArraySizeAware<QI, N> {
    fn index_mut(&mut self, index: QI) -> &mut Self::Output {
        &mut self.data[index.quant_to_usize()]
    }
}
...
impl_bitpacked_range_read_write!(
    BitPackedArraySizeAware<QI, N>,
    QI,
    [QI: QuantizedUnsignedIntegerTrait, const N: usize]
);

impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> From<[u32; N]> for BitPackedArraySizeAware<QI, N> {
    fn from(value: [u32; N]) -> Self {
        Self::from_array(value)
    }
}
...
//region Size Aware and Size Unaware conversions
//
// Default unaware → aware conversions treat every u32 word as fully utilised
// (`data.len() * 32` addressable bits). Tuple `(value, number_bits)` overloads
// allow supplying an explicit logical bit count when converting to a size-aware type.
//
...
impl<'a, QI: QuantizedUnsignedIntegerTrait> From<BitPackedSliceSizeUnaware<'a, QI>> for BitPackedSliceSizeAware<'a, QI> {
    fn from(value: BitPackedSliceSizeUnaware<'a, QI>) -> Self {
        Self::from_u32s(value.data)
    }
}

impl<'a, QI: QuantizedUnsignedIntegerTrait> From<(BitPackedSliceSizeUnaware<'a, QI>, QI)> for BitPackedSliceSizeAware<'a, QI> {
    fn from((value, number_bits): (BitPackedSliceSizeUnaware<'a, QI>, QI)) -> Self {
        Self::new(value.data, number_bits)
    }
}

impl<'a, QI: QuantizedUnsignedIntegerTrait> From<BitPackedSliceSizeAware<'a, QI>> for BitPackedSliceSizeUnaware<'a, QI> {
    fn from(value: BitPackedSliceSizeAware<'a, QI>) -> Self {
        Self::new(value.data)
    }
}

impl<'a, QI: QuantizedUnsignedIntegerTrait> From<BitPackedSliceMutSizeUnaware<'a, QI>> for BitPackedSliceMutSizeAware<'a, QI> {
    fn from(value: BitPackedSliceMutSizeUnaware<'a, QI>) -> Self {
        Self::from_u32s(value.data)
    }
}

impl<'a, QI: QuantizedUnsignedIntegerTrait> From<(BitPackedSliceMutSizeUnaware<'a, QI>, QI)> for BitPackedSliceMutSizeAware<'a, QI> {
    fn from((value, number_bits): (BitPackedSliceMutSizeUnaware<'a, QI>, QI)) -> Self {
        Self::new(value.data, number_bits)
    }
}

impl<'a, QI: QuantizedUnsignedIntegerTrait> From<BitPackedSliceMutSizeAware<'a, QI>> for BitPackedSliceMutSizeUnaware<'a, QI> {
    fn from(value: BitPackedSliceMutSizeAware<'a, QI>) -> Self {
        Self::new(value.data)
    }
}

impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> From<BitPackedArraySizeUnaware<QI, N>> for BitPackedArraySizeAware<QI, N> {
    fn from(value: BitPackedArraySizeUnaware<QI, N>) -> Self {
        Self::from_array(value.data)
    }
}

impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> From<(BitPackedArraySizeUnaware<QI, N>, QI)> for BitPackedArraySizeAware<QI, N> {
    fn from((value, number_bits): (BitPackedArraySizeUnaware<QI, N>, QI)) -> Self {
        Self::from_array_with_bits(value.data, number_bits)
    }
}

impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> From<BitPackedArraySizeAware<QI, N>> for BitPackedArraySizeUnaware<QI, N> {
