use crate::generic_collections::generic_par_data::par_data_error::{
    ParDataError, ParDataInvalidRange,
};
...
Index<QI, Output=D> + Index<Range<QI>, Output=[D]>
{
    ...
    fn subslice(&self, range: Range<QI>) -> Result<ParDataSlice<'_, QI, D>, ParDataError> {
        ...
        match self.as_slice().get(start..end) {
            ...
            None => Err(
                ParDataInvalidRange::new(
                    "subslice range is out of bounds",
                    start,
                    end,
                )
                    .into(),
...
ParData<QI, D> + IndexMut<QI, Output=D> + IndexMut<Range<QI>, Output=[D]>
{
    ...
    fn subslice_mut(
        &mut self,
        range: Range<QI>,
    ) -> Result<ParDataSliceMut<'_, QI, D>, ParDataError> {
        let start = range.start.quant_to_usize();
        let end = range.end.quant_to_usize();
