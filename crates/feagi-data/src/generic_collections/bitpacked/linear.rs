...
pub trait BitPackedAwareSizeMut<QI: QuantizedUnsignedIntegerTrait>:
BitPackedMut<QI> + BitPackedAwareSize<QI>
{
    ...
    fn subslice_u32s_mut(
        &mut self,
        range: Range<QI>,
    ) -> Result<BitPackedSliceMutSizeAware<'_, QI>, BitPackError> {
        let start = range.start.quant_to_usize();
        let end = range.end.quant_to_usize();
...
pub trait BitPackedUnawareSize<QI: QuantizedUnsignedIntegerTrait>: BitPacked<QI> {
    ...
    fn as_bit_packed_slice(
        &self,
        number_addressable_bits: QI,
    ) -> BitPackedSliceSizeAware<'_, QI> {
    ...
    fn clone_to_owned(&self, number_addressable_bits: QI) -> BitPackedVectorSizeAware<QI> {
        BitPackedVectorSizeAware::from_vec_with_bits(
            self.as_u32s().to_vec(),
            number_addressable_bits,
        )
    }
    ...
    /// eventually the last u32 word will cover a range less than 32. This function
    /// will return less than 32 if that's the case.
    fn get_number_bits_remaining_from_u32_unchecked(
        &self,
        u32_index: QI,
        number_addressable_bits: QI,
    ) -> u32 {
...
impl<QI: QuantizedUnsignedIntegerTrait> BitPackedVectorSizeUnaware<QI> {
    ...
    pub fn new_uniform_for_bits(number_bits: QI, initial_state: bool) -> BitPackedVectorSizeUnaware<QI> {
        ...
        Self::new_uniform(
            QI::quant_from_usize_unchecked(number_u32s),
            initial_state,
        )
    }
...
impl<QI: QuantizedUnsignedIntegerTrait, const N: usize> BitPackedAwareSize<QI>
