...
// NOTE: Not adding Serde support explicitly since its not a good idea to try to serialize these
// to JSON or something. TODO this actually may not be the right move when we get files, but
// we will see once we reach that point
