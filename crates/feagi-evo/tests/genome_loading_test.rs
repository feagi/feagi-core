...
fn test_load_barebones_genome() {
    ...
    let genome = load_genome_from_file(genome_path)
        .expect("Failed to load barebones genome");
...
fn test_load_all_sample_genomes() {
    ...
    for genome_file in &genome_files {
        let genome_path = format!("../../../feagi-py/feagi/evo/defaults/genome/{}", genome_file);
        ...
        match load_genome_from_file(&genome_path) {
            ...
            Err(e) => {
                println!("⚠️  Could not load {}: {} (this is OK if file doesn't exist)", 
                    genome_file, e);
            }
        }
    }
