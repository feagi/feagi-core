...
impl std::fmt::Display for ImageFrameProcessor {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        ...
        let mut steps: String = match (self.cropping_from, self.final_resize_xy_to) {
            (None, None) => format!(
                "Keeping input size of {} (no cropping from or resizing to)",
                self.input_image_properties.get_image_resolution()
            ),
            (Some(cropping_from), None) => format!(
                "Cropping from xy points {} to {} without resizing after,",
                cropping_from.upper_left, cropping_from.lower_right
            ),
            (None, Some(final_resize_xy_to)) => format!(
                "resizing to resolution {} without any cropping,",
                final_resize_xy_to
            ),
            (Some(cropping_from), Some(final_resize_xy_to)) => format!(
                "Cropping from xy points {} to {} then resizing to resolution {},",
        ...
        steps += &*(match self.offset_brightness_by {
            None => String::new(),
            Some(multiply_brightness_by) => {
                format!("Multiply brightness by {}", multiply_brightness_by)
            }
        });
...
impl ImageFrameProcessor {
    ...
    pub fn get_input_image_properties(&self) -> &ImageFrameProperties {
    ...
    pub fn verify_input_image_allowed(
        &self,
        verifying_image: &ImageFrame,
    ) -> Result<(), FeagiDataError> {
        self.input_image_properties
            .verify_image_frame_matches_properties(verifying_image)
    }
        ...
        match self {
            ...
            ImageFrameProcessor {
                ...
                to_grayscale(
                    source,
                    destination,
                    self.input_image_properties.get_color_space(),
                )
            }
            ...
            _ => {
                ...
                let mut processing =
                    ImageFrame::new_from_image_frame_properties(&final_properties)?; // TODO we can optimize this
    ...
    pub fn set_cropping_from(
        ...
        corner_points
            .verify_fits_in_resolution(self.get_output_image_properties().get_image_resolution())?;
        self.cropping_from = Some(corner_points);
        Ok(self)
    }

    pub fn set_resizing_to(
        &mut self,
        new_xy_resolution: ImageXYResolution,
    ) -> Result<&mut Self, FeagiDataError> {
        self.final_resize_xy_to = Some(new_xy_resolution);
        Ok(self)
    }

    pub fn set_brightness_offset(
        &mut self,
    ...
    pub fn set_conversion_to_grayscale(
        &mut self,
        convert_to_grayscale: bool,
    ) -> Result<&mut Self, FeagiDataError> {
...
fn crop(
    source: &ImageFrame,
    destination: &mut ImageFrame,
    crop_from: &CornerPoints,
    number_output_color_channels: usize,
) -> Result<(), FeagiDataError> {
    let destination_data = destination.get_internal_data_mut();
    let sliced_array_view: ArrayView3<u8> = source.get_internal_data().slice(s![
        crop_from.upper_left.y as usize..crop_from.lower_right.y as usize,
...
fn resize(source: &ImageFrame, destination: &mut ImageFrame) -> Result<(), FeagiDataError> {
    ...
    Zip::indexed(destination.get_internal_data_mut()).par_for_each(|(y, x, c), out| {
...
fn crop_and_resize_and_grayscale(
    source: &ImageFrame,
    destination: &mut ImageFrame,
    crop_from: &CornerPoints,
    _resize_xy_to: &ImageXYResolution,
    output_color_space: ColorSpace,
) -> Result<(), FeagiDataError> {
    let number_output_color_channels = source.get_color_channel_count();
    ...
    let dest_gray_channel = destination
        .get_internal_data_mut()
        .slice_mut(ndarray::s![.., .., 0]);

    Zip::indexed(dest_gray_channel).par_for_each(|(y, x), out| {
        let src_y = (y as f32 * scale_y).floor() as usize;
        let src_x = (x as f32 * scale_x).floor() as usize;
