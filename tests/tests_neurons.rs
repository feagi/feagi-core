...
fn test_minimal_memory_corruption_debug() {
    ...
    let mut neurons = NeuronVoxelXYZPSparseVectors::with_capacity(1);
    neurons.push(&neuron);
...
fn test_serialize_deserialize_neuron_mapped_areas() {
    ...
    let mut neurons_a = NeuronVoxelXYZPSparseVectors::with_capacity(2); // lets preallocate
    ...
    let mut neurons_b = NeuronVoxelXYZPSparseVectors::with_capacity(1); // incorrect preallocation (system should grow)
    ...
    let neurons_c = NeuronVoxelXYZPSparseVectors::new_from_ndarrays(
        neurons_c_x,
        neurons_c_y,
        neurons_c_z,
        neurons_c_p,
    ...
    let received_cortical_mappings =
        CorticalMappedXYZPNeuronVoxels::try_from(received_boxed).unwrap();

    assert_eq!(received_cortical_mappings.len(), 3);
    assert!(received_cortical_mappings.contains_cortical_id(&cortical_id_a));
    assert!(received_cortical_mappings.contains_cortical_id(&cortical_id_b));
    assert!(received_cortical_mappings.contains_cortical_id(&cortical_id_c));
