...
fn run_profiling_test(with_projection: bool) -> PerformanceMetrics {
    ...
    let _progress = {
        ...
        bdu_lock.load_from_genome(genome).expect("Failed to load genome")
    };
    
    println!("   ✓ Genome loaded");
    println!("   ✓ Burst frequency: {} Hz ({:.1}ms per burst)", BURST_FREQUENCY_HZ, 1000.0 / BURST_FREQUENCY_HZ);
    println!("   ✓ Area size: {}x{}x{} = {} neurons", AREA_SIZE.0, AREA_SIZE.1, AREA_SIZE.2, AREA_SIZE.0 * AREA_SIZE.1 * AREA_SIZE.2);
    if with_projection {
    ...
    for _ in 0..50 {
        inject_power_stimulus(&npu, STIMULUS_NEURONS);
...
fn inject_power_stimulus(npu: &Arc<Mutex<DynamicNPU>>, neuron_count: usize) {
    ...
    // Generate random XYZP data
    let mut xyzp_data = Vec::with_capacity(neuron_count);
    for _ in 0..neuron_count {
        let flat_id = rng.gen_range(0..max_id) as u32;
        let x = flat_id % AREA_SIZE.0;
        let y = flat_id / AREA_SIZE.0;
        let z = 0;
        let potential = rng.gen_range(50.0..100.0);
...
fn print_metrics(label: &str, metrics: &PerformanceMetrics) {
    ...
    println!("   {}", "-".repeat(76));
    println!("   Total duration:        {:>10.2} s", metrics.total_duration.as_secs_f32());
    println!("   Total bursts:          {:>10}", metrics.burst_times.len());
    println!("   Achieved frequency:    {:>10.2} Hz (target: {} Hz)", metrics.achieved_hz(), BURST_FREQUENCY_HZ);
    println!();
    println!("   Burst Timing:");
    println!("     Average:             {:>10.2} ms", metrics.avg_burst_time().as_secs_f32() * 1000.0);
    println!("     Min:                 {:>10.2} ms", metrics.min_burst_time().as_secs_f32() * 1000.0);
    println!("     Max:                 {:>10.2} ms", metrics.max_burst_time().as_secs_f32() * 1000.0);
    println!("     P95:                 {:>10.2} ms", metrics.p95_burst_time().as_secs_f32() * 1000.0);
    println!("     P99:                 {:>10.2} ms", metrics.p99_burst_time().as_secs_f32() * 1000.0);
    println!();
    println!("   Phase Breakdown:");
    println!("     Injection (avg):     {:>10.2} ms ({:>5.1}%)", 
        metrics.avg_inject_time().as_secs_f32() * 1000.0,
        (metrics.avg_inject_time().as_secs_f32() / metrics.avg_burst_time().as_secs_f32()) * 100.0
    );
    println!("     Execution (avg):     {:>10.2} ms ({:>5.1}%)", 
...
fn compare_metrics(baseline: &PerformanceMetrics, projection: &PerformanceMetrics) {
    ...
    println!("     Overhead:            {:>10.2} ms (+{:.1}%)", overhead_ms, overhead_pct);
    ...
    println!("     Injection phase:     {:>10.2} ms → {:>10.2} ms (+{:.2} ms)", 
        baseline_inject, projection_inject, inject_overhead);
    println!("     Execution phase:     {:>10.2} ms → {:>10.2} ms (+{:.2} ms)", 
    ...
    println!("     Projection P95:      {:>10.2} ms (+{:.2} ms)", projection_p95, projection_p95 - baseline_p95);
    println!("     Baseline P99:        {:>10.2} ms", baseline_p99);
    println!("     Projection P99:      {:>10.2} ms (+{:.2} ms)", projection_p99, projection_p99 - baseline_p99);
}
...
fn analyze_bottlenecks(baseline: &PerformanceMetrics, projection: &PerformanceMetrics) {
    ...
    println!("   Target burst time:     {:.2} ms (for {} Hz)", target_burst_time, BURST_FREQUENCY_HZ);
    ...
    if projection_burst_time > target_burst_time {
        ...
        println!("   ⚠️  BOTTLENECK DETECTED: {:.2} ms over budget ({:.1}%)", 
            time_over, (time_over / target_burst_time) * 100.0);
        println!();
        
        // Identify primary bottleneck
        if exec_overhead > 5.0 {
            println!("   🔍 PRIMARY BOTTLENECK: Burst Execution Phase");
            println!("      - Execution overhead: +{:.2} ms ({:.1}%)", 
                exec_overhead, (exec_overhead / baseline_exec) * 100.0);
            println!("      - Likely causes:");
            println!("        • Projection/propagation processing ({}K neurons)", 
            ...
            println!("      1. Profile propagation engine with {}x{} projection", AREA_SIZE.0, AREA_SIZE.1);
            ...
            println!("      - Injection overhead: +{:.2} ms ({:.1}%)", 
                inject_overhead, (inject_overhead / baseline_inject) * 100.0);
            println!("      - Likely causes:");
        ...
        println!("   ✅ NO BOTTLENECK: System can sustain {} Hz with projection", BURST_FREQUENCY_HZ);
        println!("      - Projection overhead: +{:.2} ms ({:.1}%)", exec_overhead, (exec_overhead / baseline_exec) * 100.0);
        println!("      - System has {:.2} ms headroom", target_burst_time - projection_burst_time);
    }
}
