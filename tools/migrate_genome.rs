...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Get genome metadata
    let genome_id = genome_json.get("genome_id")
        .and_then(|v| v.as_str())
        .unwrap_or("unknown");
    let version = genome_json.get("version")
        .and_then(|v| v.as_str())
        .unwrap_or("unknown");
    ...
    println!("   ✅ Migrated {} cortical IDs", migration_result.cortical_ids_migrated);
    println!();
    ...
    if let Some(genome_obj) = migrated_genome.as_object_mut() {
        ...
        let mut description = genome_obj.get("genome_description")
            .and_then(|v| v.as_str())
            .unwrap_or("")
            .to_string();
