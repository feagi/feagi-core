...
**Navigation model:** a **single-focus wizard** (not one scroll-everything page) with seven steps:

1. **Training setup** — visual **data-type tiles** derived from `TaskTemplate.group` (`Tables`, `Images`, `Signals`, `Robots`, `Mixed`), then 1–3 short **task cards** from `list_task_templates(experiment_context)`. Tiles are shape families, not dataset brands. Selecting a card pre-fills one `RunConfig` draft (not parallel). Protocol template, phases, sampler seed, and multi-seed repeats sit behind **Advanced**. CSV / folder import remains a **dataset source** on Step 2. The stepper stays hidden until a task is chosen. Wireframe mock: `feagi-desktop/src/plugins/trainer/taskTemplates.ts`.
2. **Dataset source** — the step lists **supported-corpus tiles for the committed task only** (Segment images → Cityscapes; Classify ECG beats → MIT-BIH; Classify table rows → Fisher's Iris). Tiles do not switch the task and do not mix tables, signals, and images. Import steps belong to the selected tile. Catalog / local-package tabs remain for Experience Catalog packages. Catalog selection resolves producer-assigned `dataset_asset_id` / version / `content_hash` (ADR-012, Experience Capture ADR-006). After a local CSV or folder is chosen, the same step shows a page of raw frames from that path (`preview_trainer_dataset` with `frame_offset` / `frame_count`). When the path matches a known public corpus (MIT-BIH `mitdb`, Fisher's Iris fixture, Cityscapes root or official zip-extract folder names), an info control opens the sourced dataset write-up; unrecognized folders get no invented description. The same step shows a fact card of measured preview stats (record/row counts, eligible-window class tallies) plus official highlights only when identity is established. Image segmentation adds a class map on this step: each Cityscapes trainId is Keep, Merge into a kept class, or Ignore; surviving Keep classes are compacted to IPU Z `0..K-1` and become `class_count` / mask depth.
...
- **ActiveExperimentWidget** (AppBar): embodiment thumbnail, genome title, session running indicator; popover with session/experiment ids and a single **Start experiment** action for the associated experiment (does not open My Experiments).
- **Precondition strip:** session, FEAGI burst, selected dataset, connectome hash status.

**Composer persistence (host policy, not crate):** wizard selections for the associated experiment are stored on **`experiment.params.trainer`** (`feagi.desktop.trainer` v1), using the same read-merge-write path as MuJoCo and xARM plugin settings. Loading that experiment restores template, dataset paths, encoding, cortical names, and the rest of the wizard without re-selecting. A long-running training campaign on one experiment session is separately modeled as **`trainer_protocol`** (ordered Train/Validate/Test phases on one dataset) containing multiple **`trainer_run`** records (one crate execution per phase), each linking to a terminal **Scorecard** on `feagi_sessions.scorecards[]`. Progress snapshots for long runs are host-owned (desktop → Composer PATCH), not part of the open crate.
...
| Configure → validate → run → artifacts | Seven-step wizard; Run step is config-free |
...
Snapshot population coding fires a voxel at the bin corresponding to the value, with the potential (P) channel hardcoded to `1.0`, on **Miscellaneous IPU** (not Count Input). In neural-coding terms this is **population / positional coding with a single spike per input dimension**, with a configurable bin count and linear inverted Z. Count Input remains a percentage/count sensor type and is not the Trainer snapshot target.
