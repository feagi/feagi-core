...
6. **Run** — phase-aware live view driven by `RunEvent` (progress, warnings, interim/aggregate metrics, cancel). Schema check is **Validating**; dataset ingest + FEAGI contact is **Starting**. After cancel, failure, or completion the operator can **Restart protocol** (requires a live experiment session).
